...
a
#   particular range of versions. If only one version is given, then 
exactly
...
#
#   Versions are specified as three-digit integers whose first digit is 
the
#   major version and last two are the minor version (the same format as
#   LUA_VERSION_NUM in lua.h); e.g. 501 for Lua 5.1. The revision (e.g. 
the
...
using
#   or distributing such scripts, even though portions of the text of 
the
...
AC_DEFUN([AX_LUA_VERSION],
  ...
  if test "x$LUA" != x; then
    lua_text_version=$(LUA_INIT= $LUA -e 'print(_VERSION)' 2>&1 | cut 
-d' ' -f2)
    ...
    *)
    ...
    if test $lua_version -ge "$lua_min_version" && test $lua_version -lt 
"$lua_max_version"; then
      AC_MSG_RESULT([yes])
    else
      AC_MSG_RESULT([no])
...
int main()
{
  ...
  if (LUA_VERSION_NUM >= $lua_min_version && LUA_VERSION_NUM < 
$lua_max_version)
    exit(EXIT_SUCCESS);
  exit(EXIT_FAILURE);
}
...
AC_DEFUN([AX_LUA_READLINE],
  [AX_LIB_READLINE
  if test -n "$ac_cv_header_readline_readline_h" && test -n 
"$ac_cv_header_readline_history_h"; then
