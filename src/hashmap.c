...
bool hm_hasnext(hashmap *hm) { return hm->pos < hm->size && hm->table[hm->pos].data; }

void sm_insert(hashmap *hash, const void *data, const char *key) { hm_insert(hash, data, hash_key(key)); }
...
void *sm_remove(hashmap *hash, const char *key) { return hm_remove(hash, hash_key(key)); }
...
void *sm_get(hashmap *hash, const char *key) { return hm_get(hash, hash_key(key)); }
...
long hm_count(hashmap *hash) { return hash->count; }
