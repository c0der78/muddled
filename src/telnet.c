		...
		switch ( buf[i + 1] )
		{
			...
			case SB:
					 ...
					 && buf[i + 2] != '\0' )
				{
					...
					size_t term_type_starts = i;	// record the start of the terminal name
							  ...
							  && buf[i + 1] == '\0' ) && buf[i] != SE )
					{
						...
						if ( buf[i] < 0x1F )
						{
							...
							buf[i] = '?';	// replace any control characters
						}
					...
					else
					{
						...
						// we know we have an SE within buf[i], due to the code above
						...
						else
						{
							...
							// we have IAC SB TELOPT_TTYPE IS ... IAC SE
							//   term_type_start points at the first character after the IS.
							//   i points at the SE.
							buf[i - 1] = '\0';


								for (size_t j = term_type_starts; buf[j]; j++ )
								{
							...
							free_str_dup( &conn->
										  termType,
										  ( const char
											* ) &buf[term_type_starts] );

							log_trace( "TTYPE %s", conn->termType );
				...
				else if ( buf[i] == TELOPT_NAWS )
				{
					...
					size_t naws_starts = i;	// record the start of the terminal name
					...
					else
					{
						...
						else
						{
							...
							// we have IAC SB TELOPT_NAWS ... IAC SE
							//   term_type_start points at the first character after the IS.
							//   i points at the SE.
							buf[i - 1] = '\0';
							...
							conn->scrWidth = URANGE( 10, x, 250 );

							conn->scrHeight = URANGE( 10, y, 250 );

							log_trace( "NAWS %dx%d",
									   conn->scrWidth, conn->scrHeight );
			...
			default:			// we dont know how to handle it, assume it is IAC something something
				...
				else
				{
					...
					log_warn( "ignoring IAC %d %d", buf[i + 1], buf[i + 2] );
