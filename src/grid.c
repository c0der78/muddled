...
static void next_row(Grid *grid) {
  ...
  if (++grid->y >= grid->rowcount) {
    grid->rowcount++;
    grid->rows = (GridRow *)realloc(grid->rows, grid->rowcount * sizeof(GridRow));
...
void grid_set_next_row(Grid *grid, int columns) {
  ...
  row->columns = columns;
  row->columnsize = grid->width / columns;
  row->cells = (GridCell *)realloc(row->cells, columns * sizeof(GridCell));
...
int grid_add_default(Grid *grid, const char *value) { return grid_add(grid, ALIGN_LEFT, 1, 0, 0, 1, 1, value); }

int grid_add_border(Grid *grid, Border *border, const char *value) {
  return grid_add(grid, ALIGN_LEFT, 1, border, 0, 1, 1, value);
}

int grid_add(Grid *grid, align_t align, int colspan, Border *border, const char *color, int lm, int rm,
             const char *value) {
  if (grid->x >= grid->rows[grid->y].columns) {
    next_row(grid);
  }
  ...
  int rval = grid->x;
  strncpy(grid->rows[grid->y].cells[grid->x].value, value ? value : str_empty, BUF_SIZ - 1);
  grid->x += colspan;
  return rval;
}

void grid_set(Grid *grid, int x, int y, align_t align, int colspan, Border *border, const char *color, int left_mar,
              int right_mar) {
  if (y < 0 || y > grid->rowcount || x < 0 || x > grid->rows[grid->y].columns) {
    return;
  }
  ...
  if (border) {
    ...
    strncpy(cell->border.color, border->color, sizeof(cell->border.color));
  }
...
int grid_addf(Grid *grid, align_t align, int colspan, Border *border, const char *color, int lm, int rm,
              const char *fmt, ...) {
  va_list args;
  char buf[BUF_SIZ] = {0};
  va_start(args, fmt);
...
void grid_print(Grid *grid, Character *ch) {
  ...
  for (int i = 0; i < grid->rowcount; i++) {
    for (int j = 0; j < grid->rows[i].columns; j += grid->rows[i].cells[j].span) {
      GridRow *row = &grid->rows[i];
      GridCell *cell = &row->cells[j];
      Border *border = &cell->border;
      int width_adjust = cell->left_margin + cell->right_margin;

      if (border->flags & BORDER_LEFT) {
        xwritef(buf, "%s%c~x", border->color, border->vertical);
        width_adjust++;
      }
      ...
      int colwidth = (row->columnsize * cell->span) - (width_adjust);

      if (j + cell->span >= row->columns) {
        int loss = (grid->width - (row->columnsize * row->columns));
        colwidth += loss;
      }
      xwrite(buf, align_string(cell->align, colwidth, cell->color,
                               (border->flags & BORDER_INLINE) ? &border->horizontal : 0, cell->value));
