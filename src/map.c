...
} sector_symbols[] = {{SECT_INSIDE, "~w", 'o'},
                      {SECT_CITY, "~W", 'o'},
                      {SECT_FIELD, "~G", '*'},
                      {SECT_FOREST, "~g", '*'},
                      {SECT_HILLS, "~y", '!'},
...
void map_exits(int depth, Character *ch, Room *pRoom, int x, int y) {
  ...
  for (door = 0; door < MAX_DIR; door++) {
    ...
    if ((depth < MAXDEPTH) &&
        ((automap[roomx][roomy].pRoom == pExit->to.room) || (automap[roomx][roomy].pRoom == NULL))) {
...
void show_map(Character *ch, const char *text, bool smallMap) {
  ...
  if (smallMap) {
    if (!ch->pc || !is_explorable(ch->inRoom)) {
      ...
      int rcnt = areacount(ch->pc->explored, ch->inRoom->area);

      double rooms = (double)(arearooms(ch->inRoom->area));

      double percent = UMIN(rcnt / (rooms / 100.0), 100.0);
      sprintf(buf, "~R+-----[~x%3.0f%%~R]+%s ", percent, lcolor);
    }
  ...
  for (y = m; y <= (MAPY * 2) - m; y++) {
    ...
    for (x = n; x <= (MAPX * 2) - n; x++) {
      if (automap[x][y].pRoom) {
        if (automap[x][y].symbol == sector_symbols[automap[x][y].pRoom->sector].symbol && ch->pc &&
            is_set(ch->pc->explored, automap[x][y].pRoom->id)) {
          if (automap[x][y].pRoom->exits[DIR_UP] && automap[x][y].pRoom->exits[DIR_DOWN]) {
            automap[x][y].symbol = 'B';
          }

          else if (!automap[x][y].pRoom->exits[DIR_UP] && automap[x][y].pRoom->exits[DIR_DOWN]) {
        ...
        if (!smallMap)
          sprintf(buf + strlen(buf), " %s%c~D", sector_symbols[automap[x][y].pRoom->sector].color,
                  automap[x][y].symbol);

        else
          sprintf(buf + strlen(buf), "%s%c", sector_symbols[automap[x][y].pRoom->sector].color,
                  automap[x][y].symbol != '.' ? automap[x][y].symbol : ' ');
      }
  ...
  if (!smallMap)
    xwritelnf(ch, "%s\n\r%s%s", fillstr(NULL, scrwidth(ch)), buf, fillstr(NULL, scrwidth(ch)));
