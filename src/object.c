...
static bool canUseFloat(Character *ch) { return class_table[*ch->classes].fMana; }

static bool canUseShield(Character *ch) { return !class_table[*ch->classes].fMana; }

const wear_table_type wear_table[] = {{WEAR_NONE, WEAR_NONE, "not worn", 0},
...
const Lookup object_types[] = {{"light", OBJ_LIGHT},
                               {"scroll", OBJ_SCROLL},
                               {"wand", OBJ_WAND},
                               {"staff", OBJ_STAFF},
                               {"weapon", OBJ_WEAPON},
...
const Lookup wear_types[] = {
    {"none", WEAR_NONE},           {"head", WEAR_HEAD},      {"neck", WEAR_NECK},      {"eyes", WEAR_EYES},
    {"shoulders", WEAR_SHOULDERS}, {"back", WEAR_BACK},      {"arms", WEAR_ARMS},      {"hands", WEAR_HANDS},
    {"lwrist", WEAR_WRIST},        {"rwrist", WEAR_WRIST_2}, {"rfinger", WEAR_FINGER}, {"lfinger", WEAR_FINGER_2},
    {"torso", WEAR_TORSO},         {"waist", WEAR_WAIST},    {"legs", WEAR_LEGS},      {"feet", WEAR_FEET},
    {"about", WEAR_ABOUT},         {"wield", WEAR_WIELD},    {"dual", WEAR_DUAL},      {"shield", WEAR_SHIELD},
    {"light", WEAR_LIGHT},         {"hold", WEAR_HOLD},      {"float", WEAR_FLOAT},    {0, 0}};

const Lookup wear_flags[] = {{"none", WEAR_NONE},           {"head", WEAR_HEAD},
                             {"neck", WEAR_NECK},           {"eyes", WEAR_EYES},
                             {"shoulders", WEAR_SHOULDERS}, {"back", WEAR_BACK},
                             {"arms", WEAR_ARMS},           {"hands", WEAR_HANDS},
                             {"wrist", WEAR_WRIST},         {"finger", WEAR_FINGER},
                             {"torso", WEAR_TORSO},         {"waist", WEAR_WAIST},
                             {"legs", WEAR_LEGS},           {"feet", WEAR_FEET},
                             {"about", WEAR_ABOUT},         {"wield", WEAR_WIELD},
                             {"dual", WEAR_DUAL},           {"shield", WEAR_SHIELD},
                             {"light", WEAR_LIGHT},         {"hold", WEAR_HOLD},
                             {"float", WEAR_FLOAT},         {0, 0}};

const Lookup weapon_types[] = {{"exotic", WEAPON_EXOTIC},
                               {"sword", WEAPON_SWORD},
...
const Lookup weapon_flags[] = {
    {"flaming", WEAPON_FLAMING},   {"freezing", WEAPON_FROST}, {"vampiric", WEAPON_VAMPIRIC},
    {"sharp", WEAPON_SHARP},       {"vorpal", WEAPON_VORPAL},  {"two_handed", WEAPON_TWO_HANDS},
    {"shocking", WEAPON_SHOCKING}, {"poison", WEAPON_POISON},  {0, 0}};

const Lookup container_flags[] = {{"closeable", CONT_CLOSEABLE}, {"pickproof", CONT_PICKPROOF}, {"closed", CONT_CLOSED},
                                  {"locked", CONT_LOCKED},       {"put_on", CONT_PUT_ON},       {0, 0}};

const Lookup object_flags[] = {{"glow", ITEM_GLOW},
                               {"hum", ITEM_HUM},
                               {"dark", ITEM_DARK},
                               {"evil", ITEM_EVIL},
                               {"invis", ITEM_INVIS},
...
int load_obj_columns(Object *obj, sql_stmt *stmt) {
  ...
  for (int i = 0; i < count; i++) {
    ...
    if (!str_cmp(colname, "name")) {
      ...
      if (obj->carriedBy && obj->carriedBy->id != sql_column_int(stmt, i))
...
int save_object(Object *obj) {
  field_map obj_values[] = {{"name", &obj->name, SQL_TEXT, 0},
...
void extract_obj(Object *obj) {
  ...
  for (Object *obj_next, *obj_content = obj->contains; obj_content; obj_content = obj_next) {
...
bool can_wear(Object *obj, wear_type loc) { return wear_table[loc].flags == obj->wearFlags; }

Object *get_obj_carry(const Character *ch, const char *argument, const Character *viewer) {
  char arg[BUF_SIZ];
  Object *obj;
  long number;
  int count;
  ...
  for (obj = ch->carrying; obj != NULL; obj = obj->next_content) {
    if (obj->wearLoc == WEAR_NONE && (can_see_obj(viewer, obj)) && is_name(arg, obj->name)) {
      if (++count == number) {
        return obj;
      }
    }
...
Object *get_obj_wear(const Character *ch, const char *argument) {
  ...
  for (obj = ch->carrying; obj != NULL; obj = obj->next_content) {
    if (obj->wearLoc != WEAR_NONE && can_see_obj(ch, obj) && is_name(arg, obj->name)) {
      if (++count == number) {
        return obj;
      }
    }
