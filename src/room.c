...
const Lookup sector_table[] = {{"inside", SECT_INSIDE},
                               {"city", SECT_CITY},
                               {"field", SECT_FIELD},
                               {"forest", SECT_FOREST},
                               {"hills", SECT_HILLS},
...
const Lookup room_flags[] = {{"noexplore", ROOM_NOEXPLORE}, {"safe", ROOM_SAFE}, {"no_recall", ROOM_NO_RECALL}, {0, 0}};
...
void destroy_room(Room *room) {
  ...
  for (Character *ch_next, *ch = room->characters; ch != 0; ch = ch_next) {
    ...
    xwriteln(ch, "The gods wreack havok upon the land and you are transported somewhere safe..");
    char_from_room(ch);
...
void load_room_columns(Room *room, sql_stmt *stmt) {
  ...
  for (int i = 0; i < count; i++) {
    ...
    if (!str_cmp(colname, "name")) {
      ...
      if (room->area != 0) {
        if (room->area->id != areaId) log_error("sql returned invalid room for area");
      }
      ...
      parse_flags(room->flags, sql_column_str(stmt, i), room_flags);
    }
