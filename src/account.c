...
const Lookup account_flags[] = {{"coloroff", ACC_COLOR_OFF}, {0, 0}};
...
AccountPlayer *new_account_player() {
  AccountPlayer *p = (AccountPlayer *)alloc_mem(1, sizeof(AccountPlayer));
  p->name = str_empty;
  return p;
}
  ...
  do {
    ...
    for (i = 0; i < cols; i++) {
      ...
      if (!str_cmp(colname, "accountId")) {
        ...
        parse_flags(acc->flags, sql_column_str(stmt, i), account_flags);
      }
...
field_map *account_forum_fields(Account *acc, AccountForum *forum) {
  ...
  field_map account_values[] = {{"accountForumId", &forum->id, SQL_INT},
...
field_map *account_fields(Account *acc) {
  static field_map *table = 0;
  field_map accvalues[] = {{"accountId", &acc->id, SQL_INT},
...
int delete_account(Account *acc) { return db_delete(ACCOUNT_TABLE, acc->id) == SQL_OK; }
...
int load_account_forums(Account *acc) {
  char buf[400];
  sql_stmt *stmt;
  int len = sprintf(buf, "select * from account_forum where accountId=%" PRId64, acc->id);
