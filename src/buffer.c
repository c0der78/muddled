 ...
 * valid states
 */
enum buf_t { BUFFER_SAFE, BUFFER_OVERFLOW };
...
bool buf_addln(Buffer *buffer, const char *string) { return buf_add(buffer, string) && buf_add(buffer, "\n\r"); }
...
bool buf_add(Buffer *buffer, const char *string) { return buf_add_len(buffer, string, strlen(string)); }
