...
const Lookup area_flags[] = {{"noexplore", AREA_NOEXPLORE}, {"changed", AREA_CHANGED}, {0, 0}};
...
Area *load_area(identifier_t id) {
  ...
  sql_stmt *stmt;
  Area *area = 0;
  int len = sprintf(buf, "select * from area where areaId = %" PRId64 " limit 1", id);
  ...
  if (sql_step(stmt) != SQL_DONE) {
    ...
    log_info("loaded %s (%d npcs, %d objs, %d rooms)", area->name, load_npcs(area), load_objects(area),
             load_rooms(area));
...
int save_area_only(Area *area) {
  ...
  field_map area_values[] = {{"areaId", &area->id, SQL_INT},
