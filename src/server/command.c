...
const Command cmd_table[] = {{"north", 0, do_north, COMMAND_MANIP, POS_STANDING},
...
const Lookup command_types[] = {{"info", COMMAND_INFO},       {"settings", COMMAND_SETTINGS},
                                {"admin", COMMAND_ADMIN},     {"manipulation", COMMAND_MANIP},
                                {"connection", COMMAND_CONN}, {"communication", COMMAND_COMM},
                                {"combat", COMMAND_COMBAT},   {0, 0}};
...
void command_interpret(Character *ch, const char *argument) {
  ...
  for (cmd = cmd_table; cmd->name != 0; cmd++) {
    if (UPPER(arg[0]) == UPPER(cmd->name[0]) && !str_prefix(arg, cmd->name) && cmd->level <= ch->level) {
      found = true;
      break;
    }
  }
  ...
  if (!found) {
    ...
    const char *const message[] = {"Huh?",
                                   ...
                                   "I do not understand..."};
    xwriteln(ch, message[number_range(0, (sizeof(message) / sizeof(message[0])) - 1)]);
    return;
  }

  if (cmd->position < ch->position) {
    xwritelnf(ch, "You can't do that while you are %s.", position_table[ch->position].name);
    return;
  }
...
DOFUN(quit) {
  ...
  const char *const chmessage[] = {"Alas, all good things must come to an end.",
  ...
  long number = number_range(1, sizeof(chmessage) / sizeof(chmessage[0])) - 1;
...
DOFUN(logout) {
  ...
  ch->pc->conn->handler = (void (*)(Connection *, const char *))client_account_menu;
}
...
DOFUN(autologin) {
  ...
  if (nullstr(argument)) {
    for (p = ch->pc->account->players; p; p = p->next) {
      xwritelnf(ch, "%s~Y%2d)~C %s", p->charId == ch->pc->account->autologinId ? "~R*" : " ", ++count, p->name);
    }
...
DOFUN(north) { move_char(ch, DIR_NORTH); }

DOFUN(east) { move_char(ch, DIR_EAST); }

DOFUN(south) { move_char(ch, DIR_SOUTH); }

DOFUN(west) { move_char(ch, DIR_WEST); }

DOFUN(up) { move_char(ch, DIR_UP); }

DOFUN(down) { move_char(ch, DIR_DOWN); }
...
void wear_obj(Character *ch, Object *obj, bool fReplace) {
  if (ch->level < obj->level) {
    xwritelnf(ch, "You must be level %d to use this object.", obj->level);
    act(TO_ROOM, ch, obj, 0, "$n tries to use $p, but is too inexperienced.");
    return;
  }
  ...
  if (t != 0 && t->canUse != 0 && !(*t->canUse)(ch)) {
    ...
    act(TO_ROOM, ch, obj, 0, "$n tries to use $p, but doesn't know how.");
    return;
  }
  ...
  switch (obj->wearFlags) {
    ...
    case WEAR_WRIST:
      ...
      if (get_eq_char(ch, WEAR_WRIST) != 0 && get_eq_char(ch, WEAR_WRIST_2) != 0 &&
          !remove_obj(ch, WEAR_WRIST, fReplace) && !remove_obj(ch, WEAR_WRIST_2, fReplace)) {
        return;
      }

      if (get_eq_char(ch, WEAR_WRIST) == 0) {
        act(TO_ROOM, ch, obj, 0, "$n wears $p on $s left wrist.");
        act(TO_CHAR, ch, obj, 0, "You wear $p on your left wrist.");
        equip_char(ch, obj, WEAR_WRIST);
        return;
      }

      if (get_eq_char(ch, WEAR_WRIST_2) == 0) {
        act(TO_ROOM, ch, obj, 0, "$n wears $p on $s right wrist.");
        act(TO_CHAR, ch, obj, 0, "You wear $p on your right wrist.");
        equip_char(ch, obj, WEAR_WRIST_2);
        return;
      }
      xwriteln(ch, "You are already wearing something on your wrists.");
      return;
    case WEAR_FINGER:

      if (get_eq_char(ch, WEAR_FINGER) != 0 && get_eq_char(ch, WEAR_FINGER_2) != 0 &&
          !remove_obj(ch, WEAR_FINGER, fReplace) && !remove_obj(ch, WEAR_FINGER_2, fReplace)) {
        return;
      }

      if (get_eq_char(ch, WEAR_FINGER) == 0) {
        act(TO_ROOM, ch, obj, 0, "$n wears $p on $s left finger.");
        act(TO_CHAR, ch, obj, 0, "You wear $p on your left finger.");
        equip_char(ch, obj, WEAR_FINGER);
        return;
      }

      if (get_eq_char(ch, WEAR_FINGER_2) == 0) {
        act(TO_ROOM, ch, obj, 0, "$n wears $p on $s right finger.");
        act(TO_CHAR, ch, obj, 0, "You wear $p on your right finger.");
