...
void exit_editor_menu(Client *conn) {
  ...
  xwritelnf(conn, "~YA) ~CToRoom: ~W%d~x", exit->to.room == 0 ? 0 : exit->to.room->id);
...
void exits_editor_menu(Client *conn) {
  ...
  if (conn->editing->next != 0)
    conn->titlef(conn, "Exits Editor - Room %d", ((Room *)conn->editing->next->data)->id);
  ...
  for (const Lookup *t = direction_table; t->name != 0; t++) {
    ...
    xwritelnf(conn, "  ~Y%d) %s~C -> Room %d (%s)~x", ++count, capitalize(t->name), exits[t->value]->to.room->id,
...
void exits_editor(Client *conn, const char *argument) {
  ...
  if (!str_cmp(arg, "A") || !str_prefix(arg, "create")) {
    ...
    if (dir == -1) {
      xwritelnf(conn, "~C'%s' is not a valid direction.~x", argument);
      return;
    }

    if (exits[dir] != 0) {
      xwriteln(conn, "~CThere is already an exit in that direction.~x");
      return;
    }
    exits[dir] = new_exit();
  ...
  if (!str_cmp(arg, "D") || !str_prefix(arg, "delete")) {
    ...
    for (dir = direction_table; dir->name != 0; dir++) {
      ...
      if ((num != 0 && num == count) || !str_prefix(argument, dir->name)) {
        break;
      }
    }
    ...
    if (exits[dir->value] == 0) {
      xwriteln(conn, "~CThere is no exit in that direction.~x");
      return;
    }
  ...
  if (!str_cmp(arg, "E") || !str_prefix(arg, "edit")) {
    if (!argument || !*argument) {
      xwriteln(conn, "~CWhich direction do you wish to edit?~x");
      return;
    }
    int count = 0;
    const Lookup *dir;
...
void exit_editor(Client *conn, const char *argument) {
  ...
  if (!str_cmp(arg, "A") || !str_cmp(arg, "ToRoom")) {
    if (!argument || !*argument || !is_number(argument)) {
      xwriteln(conn, "~CPlease specify the id of the room you want to delete.~x");
      return;
    }
