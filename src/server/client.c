...
void initialize_client(int control) {
  ...
  if ((c->socket = accept(control, (struct sockaddr *)&c->addr, &size)) < 0) {
    log_error("accept");
    destroy_client(c);
    return;
  }
  ...
  if (getpeername(c->socket, (struct sockaddr *)&c->addr, &size) < 0) {
    ...
    from = gethostbyaddr((char *)&c->addr.sin_addr, sizeof(c->addr.sin_addr), AF_INET);
...
const char *getip(const Client *conn) { return inet_ntoa(conn->addr.sin_addr); }

bool parse_input_buffer(Client *conn) {
  int i, j, k, got_n, got_r;
  /*
...
#define DISABLE_EXTENDED_ASCII_CHARS
  ...
  if (k > 1 || conn->incomm[0] == '!') {
    if (conn->incomm[0] != '!' && strcasecmp(conn->incomm, conn->lastCommand)) {
      conn->repeat = 0;
    } else {
      if (++conn->repeat == 25) {
        log_warn("%s input spamming!", getip(conn));
      } else if (conn->repeat == 35) {
        log_warn("%s still input spamming!", getip(conn));
...
bool read_line_from_client(Client *conn) {
  ...
  for (;;) {
    ...
    nRead = recv(conn->socket, (conn->inbuf + iStart), (sizeof(conn->inbuf) - 10 - iStart), 0);
...
void pager_prompt(Client *conn) {
  ...
  for (const char *ptr = conn->showstr_head; *ptr && ptr != conn->showstr_point; ptr++)
  ...
  xwritef(conn, "\n\r(%d%%) (H)elp, (R)efresh page, (P)revious page, or hit ENTER: ", 100 * shown_lines / total_lines);
}
...
void bust_a_prompt(Client *conn) {
  ...
  if (!conn->account || !conn->account->playing || !conn->account->playing->pc) {
    xwrite(conn, "> ");
    return;
  }
    ...
    switch (*pstr) {
      ...
      case 'E':
        ...
        if (ch->fighting == 0) {
          ...
          sprintf(buf2, "%d%%", (int)percent(ch->fighting->hit, ch->fighting->maxHit));
          tmp = buf2;
        }
        break;
      case '%':
...
bool parse_color_codes(Client *conn) {
  ...
  for (pstr = conn->outbuf; *pstr && (pstr - conn->outbuf) < conn->outtop; pstr++) {
  ...
  if (!is_set(engine_info.flags, ENGINE_OFF)) {
    if (conn->showstr_point) {
      ...
      if ((victim = ch->fighting) != NULL && can_see(ch, victim)) {
        ...
        if (victim->maxHit > 0) {
          percent = (int)(victim->hit * 100 / victim->maxHit);
        } else {
          percent = -1;
        }
        ...
        if (percent >= 100) {
          ...
          strcpy(wound, "has quite a few wounds.");
        } else if (percent >= 30) {
          strcpy(wound, "has some big nasty wounds and scratches.");
        } else if (percent >= 15) {
        ...
        xwritelnf(conn, "~R%s %s~x", NAME(victim), wound);
      }
      xwriteln(conn, "");
      bust_a_prompt(conn);
    }
...
void show_string(Client *conn, const char *input) {
  ...
  if (lines < 0) {
    for (chk = conn->showstr_point; chk != conn->showstr_head; chk--) {
      if ((*chk == '\n') || (*chk == '\r')) {
        toggle = -toggle;
...
void client_wait_to_quit(Client *conn, const char *argument) { conn->handler = 0; }
...
void client_display_account_menu(Client *conn) {
  ...
  if (conn->account->players != 0) {
    ...
    for (AccountPlayer *ch = conn->account->players; ch != 0; ch = ch->next) {
      xwritelnf(conn, " ~Y%2d) ~W%s ~C(Lvl %d)~x", ++count, ch->name, ch->level);
    }
  }
  xwrite(conn, "\n\r~CYour selection?~x ");
}
...
bool client_is_playing(Client *conn) {
  return conn && conn->account && conn->account->playing && (conn->handler == client_command_parser);
}
...
void set_playing(Client *conn) {
  ...
  if (!server_rebooting) {
    clear_screen(conn);
    set_cursor(conn, 1, 1);
    log_info("%s playing %s (lvl %d)", conn->account->login, ch->name, ch->level);
    act(TO_ROOM, ch, 0, 0, "~?The nebula orion flickers and $n materializes from the fourth dimension...~x");
    xwriteln(conn, "~?You materialize from the fourth dimension...~x");
...
void client_confirm_delete_char(Client *conn, const char *argument) {
  ...
  if (strcmp(conn->account->password, crypt(argument, conn->account->login))) {
    set_playing(conn);
...
void client_delete_char(Connection *conn) {
  ...
  xwritelnf(conn, "~RAre you sure you want to delete '%s'?~x", cl->account->playing->name);
  xwrite(conn, "~CEnter your account password to continue, or hit return to cancel: ~x");
...
void client_display_class_menu(Client *conn) {
  ...
  for (int i = 0; i < max_class; i++) {
    xwritelnf(conn, "~W%12s~C: %s~x", class_table[i].name, class_table[i].description);
  }
  xwriteln(conn, "");
...
void client_get_class(Client *conn, const char *argument) {
  if (!argument || !*argument) {
    xwrite(conn, "~CPlease specify which class your character shall be: ~x");
    return;
  }
...
void client_get_race(Client *conn, const char *argument) {
  if (!argument || !*argument) {
    xwrite(conn, "~CPlease specify which race your character shall be: ~x");
    return;
  }
  ...
  memcpy(conn->account->playing->stats, race->stats, MAX_STAT * sizeof(int));
...
void client_get_char_sex(Client *conn, const char *argument) {
  if (!argument || !*argument) {
    xwriteln(conn, "~CPlease specify which sex your character shall be:~x ");
    return;
  }
  ...
  if (is_number(argument)) {
    ...
    if (t->name == 0) {
      xwritelnf(conn, "~CValid sexes are: ~W%s~x", lookup_names(sex_table));
      return;
    }
...
void client_create_new_char(Client *conn, const char *argument) {
  if (!argument || !*argument || strlen(argument) > 12 || strlen(argument) < 3) {
    xwriteln(conn, "Please enter a name between 3 and 12 characters.");
    return;
  }

  for (const char *str = argument; *str != 0; str++) {
    if (!isalpha((int)*str)) {
      xwriteln(conn, "You can only use letters for your characters name.");
      return;
    }
  }
...
void client_confirm_account_delete(Client *conn, const char *argument) {
  ...
  if (strcmp(conn->account->password, crypt(argument, conn->account->login))) {
    xwriteln(conn, "Incorrect password.  Account deletion cancelled.");
    conn->handler = client_account_menu;
    return;
  }
...
void client_account_menu(Client *conn, const char *argument) {
  if (is_number(argument)) {
    ...
    if (ch != 0) {
      ...
      if (conn->account->playing == 0) {
        xwriteln(conn, "There was a problem loading that character.");
      } else {
        xwritelnf(conn, "The nebula orion flashes and %s materializes from the 4th dimension...", ch->name);
        announce(conn->account->playing, INFO_JOIN, "$n has entered the realms.");
        set_playing(conn);
        return;
      }
    }
  ...
  switch (UPPER(argument[0])) {
    case 'C':
      ...
      break;
    case 'D':
      xwriteln(conn, "~CAre you sure you want to delete your account?~x");
...
void client_display_timezones(Client *conn) {
  xwritelnf(conn, "   ~C%-6s ~W%-30s (~c%s~W)", "Name", "City/Zone Crosses", "Time~x");
  time_t current_time = time(0);
  xwritelnf(conn, "~C%s~x", fillstr("-", conn->scrWidth));

  for (int i = 0; timezones[i].name != 0; i++) {
    xwritelnf(conn, "~Y%2d) ~C%-6s~W %-30s (~c%s~W)", i + 1, timezones[i].name, timezones[i].zone,
              str_time(current_time, i, NULL));
  }
  xwriteln(conn, "");
...
void client_create_account_password(Client *conn, const char *argument) {
  if (!argument || !*argument || strlen(argument) < 6) {
    xwriteln(conn, "Your password must be at least 6 symbols or more.");
    return;
  }
  free_str_dup(&conn->account->password, crypt(argument, conn->account->login));
...
void client_create_account(Client *conn, const char *argument) {
  switch (UPPER(argument[0])) {
    case 'Y':
      xwritelnf(conn, "Please enter a password for %s: ", conn->account->login);
      conn->handler = client_create_account_password;
      send_telopt(conn, WILL, TELOPT_ECHO);
      break;
    case 'N':
...
void client_get_account_password(Client *conn, const char *argument) {
  if (strcmp(conn->account->password, crypt(argument, conn->account->login))) {
    if (conn->password_retries >= 2) {
      send_telopt(conn, WONT, TELOPT_ECHO);
      xwrite(conn, "Too many password attempts.  Disconnecting.");
      conn->handler = 0;
      return;
    }
  ...
  if (p == 0) {
    ...
    conn->account->playing = load_player_by_id((Connection *)conn, p->charId);

    if (conn->account->playing == 0) {
      xwriteln(conn, "There was a problem loading your character.");
    } else {
...
void client_get_account_name(Client *conn, const char *argument) {
  ...
  if (!load_account(conn->account, argument)) {
    ...
    xwritef(conn, "Would you like to create one for %s? (Y/N) ", argument);
