...
ssize_t send_telopt(const Client *conn, unsigned char cmd, unsigned char opt) {
  ...
  return send(conn->socket, telopt, sizeof(telopt) / sizeof(telopt[0]), 0);
}

ssize_t test_telopts(const Client *conn) { return send_telopt(conn, DO, TELOPT_TSPEED); }
  ...
  while (!finished && buf[i] == IAC) {
    switch (buf[i + 1]) {
      ...
      case SB:
        ...
        if (buf[i] == TELOPT_TTYPE && buf[i + 1] == IS && buf[i + 2] != '\0') {
          ...
          while (!(buf[i] == '\0' && buf[i + 1] == '\0') && buf[i] != SE) {
            if (buf[i] < 0x1F) {
              buf[i] = '?';
          ...
          if (buf[i] == '\0' && buf[i + 1] == '\0') {
            ...
            if (buf[i - 1] != IAC) {
               ...
               * the buffer
               */
              free_str_dup(&conn->termType, (const char *)&buf[term_type_starts]);
          ...
          if (buf[i] == '\0' && buf[i + 1] == '\0') {
            ...
            if (buf[i - 1] != IAC) {
              ...
              log_trace("NAWS %dx%d", conn->scrWidth, conn->scrHeight);
            }
            i++;
          }
        } else {
