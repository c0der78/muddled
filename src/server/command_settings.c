...
DOFUN(timezone) {
  ...
  if (!*argument) {
    ...
    xwritelnf(buf, "%-6s %-30s (%s)", "Name", "City/Zone Crosses", "Time");
    time_t current_time = time(0);
    xwritelnf(buf, "%s~x", line);

    for (int i = 0; timezones[i].name != 0; i++) {
      xwritelnf(buf, "%-6s %-30s (%s)", timezones[i].name, timezones[i].zone, str_time(current_time, i, NULL));
    }
    xwritelnf(buf, "%s~x", line);
  ...
  if (i == -1) {
    xwritelnf(ch, "That time zone does not exists.  See '%s' for a list.",

              do_name);
    return;
  }
  ch->pc->account->timezone = i;
  xwritelnf(ch, "Your time zone is now %s %s (%s)", timezones[i].name, timezones[i].zone, str_time(time(0), i, NULL));
}
