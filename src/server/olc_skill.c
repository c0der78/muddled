...
void skill_editor_menu(Client *conn) {
  ...
  xwritelnf(conn, "~YJ) ~CMinPos: ~W%s~x", position_table[skill->minPos].name);
...
void skill_edit_list(Client *conn) {
  ...
  for (int i = 0; i < max_skill; i++) {
    xwritef(conn, "%2d) %-12.12s ", skill_table[i].id, skill_table[i].name);
    ...
    if (nullstr(argument)) {
      ...
      if (skill->spellfun == 0) {
        ...
        for (i = 0; spellfun_table[i].name != 0; i++) {
          xwritef(conn, "%-10s ", spellfun_table[i].name);
