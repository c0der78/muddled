...
DOFUN(commands) {
  ...
  ch->titlef(ch, "%s Commands", type == -1 ? "All" : capitalize(command_types[type].name));
  ...
  for (int i = 0; i < max_command; i++) {
    ...
    if (type != -1 && !(cmdlist[i].category & command_types[type].value)) {
      continue;
    }
...
void show_char_to_char_1(Character *victim, Character *ch) {
  ...
  if (!nullstr(victim->description)) {
    ...
    act(TO_CHAR, ch, 0, victim, "You see nothing special about $M.");
  }
  ...
  for (iWear = 0; wear_table[iWear].display != 0; iWear++) {
    if ((obj = get_eq_char(victim, wear_table[iWear].loc)) != NULL && can_see_obj(ch, obj)) {
      if (!found) {
        xwriteln(ch, "");
        act(TO_CHAR, ch, 0, victim, "$N is using:");
        found = true;
      }
      xwritef(ch, "~g<~w%s~g>~w ", align_string(ALIGN_CENTER, 22, 0, 0, wear_table[iWear].display));
...
DOFUN(look) {
  if (nullstr(argument)) {
    ...
    if (!ch->pc || !is_set(ch->pc->account->flags, PLR_BRIEF)) {
      if (!ch->pc || is_set(ch->pc->account->flags, PLR_AUTOMAP_OFF)) {
    ...
    for (Character *rch = ch->inRoom->characters; rch != 0; rch = rch->next_in_room) {
      if (rch == ch) {
        continue;
      }
    ...
    for (Object *obj = ch->inRoom->objects; obj != 0; obj = obj->next_content) {
...
DOFUN(time) {
  ...
  xwritelnf(ch, "Today is %s, %s day in the Month of the %s, year %d.", weekdays[tm->tm_wday],
            ordinal_string(tm->tm_wday), months[tm->tm_mon], tm->tm_year);
  xwritelnf(ch, "It is the season of %s.", seasons[tm->tm_mon / 3]);
  xwriteln(ch, "");

  if (last_reboot != 0) xwritelnf(ch, "Last reboot was on %s.", str_time(last_reboot, ch->pc->account->timezone, 0));

  if (startup_time != 0) {
    xwritelnf(ch, "Startup time was %s.", str_time(startup_time, ch->pc->account->timezone, 0));
  }
}
...
DOFUN(who) {
  ...
  for (Character *wch = first_player; wch != 0; wch = wch->next_player) {
    xwritelnf(buffer, "~W[~Y%02d ~R%3.3s ~B%3.3s~W] %s %s", wch->level, capitalize(wch->race->name), class_who(wch),
              wch->name, wch->pc->title);
  }
...
DOFUN(score) {
  ...
  grid_addf_default(grid, "~CSex: ~W%s", capitalize(sex_table[ch->sex].name));
  grid_addf_border(grid, &right, "~CPosition: ~W%s", capitalize(position_table[ch->position].name));
  grid_addf_border(grid, &left, "~CFlags: ~W%s",

                   format_flags(ch->flags, ch->pc ? plr_flags : npc_flags));
  ...
  grid_addf_border(grid, &left, "~CHit: ~R%ld~W/~R%ld", ch->hit, ch->maxHit);
  grid_addf_default(grid, "~CMana: ~M%ld~W/~M%ld", ch->mana, ch->maxMana);
  grid_addf_border(grid, &right, "~CMove: ~B%ld~W/~B%ld", ch->move, ch->maxMove);
  // grid_set_next_row(grid, 6);
  int maxTrain = getMaxTrain(ch);
  grid_addf_border(grid, &left, "~CStr: ~W%d/%d", getCurrStat(ch, STAT_STR), maxTrain);
  grid_addf_default(grid, "~CInt : ~W%d/%d", getCurrStat(ch, STAT_INT), maxTrain);
  grid_addf_border(grid, &right, "~CWis : ~W%d/%d", getCurrStat(ch, STAT_WIS), maxTrain);
  grid_addf_border(grid, &left, "~CDex: ~W%d/%d", getCurrStat(ch, STAT_DEX), maxTrain);
  grid_addf_default(grid, "~CCon : ~W%d/%d", getCurrStat(ch, STAT_CON), maxTrain);
  grid_addf_border(grid, &right, "~CLuck: ~W%d/%d", getCurrStat(ch, STAT_LUCK), maxTrain);
  ...
  if (ch->pc) {
    grid_addf_border(grid, &right, "~CTerminal: ~W%s",
                     nullstr(ch->pc->conn->termType) ? "Unknown" : ch->pc->conn->termType);
    grid_addf_border(grid, &left, "~CScreen Width: ~W%d", ch->pc->conn->scrWidth);
    grid_addf_default(grid, "~CScreen Height: ~W%d", ch->pc->conn->scrHeight);
  }
  // grid_set(grid, grid->x, grid->y, ALIGN_LEFT, 1, &right, 0, 0, 0);
  grid_addf_border(grid, &right, "~CAlignment: ~W%d", ch->alignment);

  if (!is_immortal(ch)) {
    grid_addf_border(grid, &left, "~CExp: ~W%ld", ch->pc->experience);
...
DOFUN(explored) {
  ...
  if (nullstr(argument)) {
    ...
    xwritelnf(ch, "%s has ~G%d~x explorable rooms.", engine_info.name, max_explorable_room);
    xwritelnf(ch, "You have explored ~G%d~x rooms (%.2f%%)~x of the world.", rcnt, percent);
    rcnt = areacount(ch->pc->explored, ch->inRoom->area);
    rooms = (double)(arearooms(ch->inRoom->area));
    percent = UMIN(rcnt / (rooms / 100.0), 100.0);
    xwritelnf(ch, "\n\r%s has ~G%.0f~x explorable rooms.", ch->inRoom->area->name, rooms);
    xwritelnf(ch, "You have explored ~G%d (%.2f%%)~x rooms in this area.", rcnt, percent);
    ...
    list = (struct area_index *)alloc_mem(max_area, sizeof(struct area_index));
    ...
    qsort(list, i, sizeof(struct area_index), compare_area_explored);

    for (c = 0; c < i; c++) {
      pArea = list[c].area;
      percent = list[c].percent;
      xwritelnf(output, "~D[~Y%3.0f~y%%~D]~x %s", percent, pArea->name);
    }
    ...
    cmd_syntax(ch, do_name, "       - show current area and world.", "list   - list percentages for all areas.",
...
DOFUN(equipment) {
  for (int i = 1; wear_table[i].display != 0; i++) {
    ...
    xwritef(ch, "~g<~w%s~g>~w ", align_string(ALIGN_CENTER, 22, 0, 0, wear_table[i].display));
  ...
  if (ch->affects) {
    ...
    for (paf = ch->affects; paf; paf = paf->next) {
      if (paf_last != NULL && paf->from == paf_last->from) {
        ...
        xwritef(buffer, "Spell: ~c%-19s~x", valid_skill(paf->from) ? skill_table[paf->from].name : "unknown");
      }

      if (ch->level >= 20) {
        xwritef(buffer, ": modifies %s by %d ", affect_name(paf), paf->modifier);
        ...
        if (paf->duration == -1) {
          ...
          xwritef(buffer, "for %d minutes~x", (paf->duration * (PULSE_TICK / PULSE_PER_SECOND)) / 60);
      }
      xwriteln(buffer, "");
      paf_last = paf;
    }
...
DOFUN(resists) {
  ...
  for (int i = 0; i < MAX_DAM; i++) {
    xwritelnf(buf, "~C%-12s  ~W%3d~x", capitalize(dam_types[i].name), ch->resists[dam_types[i].value]);
  }
