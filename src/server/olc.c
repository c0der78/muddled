...
void destroy_editor(Editor *editor) { free_mem(editor); }

void olc_prompt(Client *conn) {
  if (conn->editing->show == string_editor_menu) {
    xwritelnf(conn, "~CType \\? for help or \\q to %s:~x ", conn->editing->next ? "go back" : "quit");
  }

  else {
    xwritef(conn, "~CSelect an option, ? for help or Q to %s:~x ", conn->editing->next ? "go back" : "quit");
  }
}
...
void string_editor_preview(Client *conn, const char *header, const char *pstr) {
  ...
  if (pstr && *pstr) {
    ...
    while (pstr && *pstr != 0 && count < 3) {
      pstr = get_line(pstr, buf);
      xwritelnf(conn, "%-*s  ~W%2d. %s~x", len, " ", ++count, buf);
    }
  }
...
void string_lineadd(const char *string, const char *newstr, size_t line, char *buf) {
  size_t cnt = 1;
  bool done = false;
  buf = 0;

  for (const char *strtmp = string; *strtmp != 0 || (!done && cnt == line); strtmp++) {
    if (cnt == line && !done) {
      strcat(buf, newstr);
      strcat(buf, "\n\r");
      cnt++;
...
int edit_text(Client *conn, const char **pStr, const char *argument) {
  ...
  if (!str_prefix("\\d", argument)) {
    ...
    if (*argument && is_number(argument)) {
      ...
      if (line < 1 || line > lines) {
        xwritelnf(conn, "~CYou can't delete line %d, it doesn't exist.~x", line);
        return EDIT_NOCHANGE;
      }
    }
  ...
  if (!str_prefix("\\i", argument)) {
    ...
    if (!argument || !*argument || !is_number(argument)) {
      xwriteln(conn, "~CYou must specify a line to insert at.~x");
      return EDIT_NOCHANGE;
    }
...
void string_edit(Client *conn, const char *argument) {
  int rval = edit_text(conn, (const char **)conn->editing->data, argument);
...
int character_editor(Client *conn, Character *ch, const char *arg, const char *argument) {
  ...
  if (!str_cmp(arg, "C") || !str_cmp(arg, "level")) {
    ...
    if (l == 0 || l >= MAX_LEVEL * 2) {
      xwritelnf(conn, "~CA valid level is between 1 and %d.~x", MAX_LEVEL * 2);
      return 1;
    }
    ch->level = l;
    conn->editing->show(conn);
  ...
  if (!str_cmp(arg, "F") || !str_cmp(arg, "gold")) {
    if (nullstr(argument) || !is_number(argument)) {
      xwriteln(conn, "~CYou must specify a decimal number to set.~x");
      return 1;
    }
...
int edit_flag(const char *arg, Client *conn, Flag *flags, const char *argument, const Lookup *table) {
  if (nullstr(argument) || argument[0] == '?') {
    xwritelnf(conn, "~CValid flags are: ~W%s~x", lookup_names(table));
    return EDIT_NOCHANGE;
  }
...
int edit_string(const char *arg, Client *conn, const char **str, const char *argument) {
  if (nullstr(argument) || argument[0] == '?') {
