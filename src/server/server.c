...
int initialize_server() {
  if (!server_rebooting) {
...
#if defined SO_DONTLINGER && !defined SYSV
    {
      ...
      if (setsockopt(server_socket, SOL_SOCKET, SO_DONTLINGER, (char *)&ld, sizeof(ld)) < 0) {
    ...
    if (fp == 0) {
      ...
      while (fgets(buf, sizeof(buf), fp) != NULL) {
        ...
        if (sscanf(buf, "%d %d %d %[^'~']~ %[^'~']~ %s\n", &conn->socket, &width, &height, account, name, term) != 6) {
...
int game_loop(int control) {
  ...
  while (server_socket != -1) {
     ...
     */
    {
      ...
      usecDelta = last_time.tv_usec - now_time.tv_usec + 1000000 / PULSE_PER_SECOND;
...
void reboot_server() {
  ...
  for (Client *conn = first_client; conn; conn = conn->next) {
    ...
    fprintf(fp, "%d %d %d %s~ %s~ %s\n", conn->socket, conn->scrWidth, conn->scrHeight, conn->account->login,
