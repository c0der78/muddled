...
DOFUN(edit) {
  ...
  if (!argument || !*argument) {
    cmd_syntax(ch, do_name, "room", "help", "social", "npc", "object", "area", "player", "skill", "engine", 0);
    /*
  ...
  if (!str_prefix(arg, "room")) {
    ...
    cmd_syntax(ch, do_name, "room		- edit the current room",
               "room create	- create a room for the current area", "room id#	- edit a specific room",
  ...
  if (!str_prefix(arg, "help")) {
    if (!argument || !*argument || argument[0] == '?') {
      cmd_syntax(ch, do_name, "help <keyword or id#> 	- edit a specific help",
                 "help create	 	- create a help", "help list		- list helps", 0);
      return;
    }
    Help *help;
  ...
  if (!str_prefix(arg, "social")) {
    if (!argument || !*argument || argument[0] == '?') {
      cmd_syntax(ch, do_name, "social <keyword or id#> 	- edit a specific social",
                 "social create	 	- create a social", "social list		- list socials", 0);
      return;
    }
    Social *social;
  ...
  if (!str_prefix(arg, "skill")) {
    if (!argument || !*argument || argument[0] == '?') {
      cmd_syntax(ch, do_name, "skill <name or id#> 	- edit a specific skill",
                 "skill create	 	- create a skill", "skill list		- list skills", 0);
      return;
    }
    Skill *skill;
  ...
  if (!str_prefix(arg, "player")) {
    if (!argument || !*argument || argument[0] == '?') {
      cmd_syntax(ch, do_name, "player <keyword or id#> 	- edit a specific player",
                 "player list			- list players", 0);
      return;
    }
    Character *vch;
  ...
  if (!str_prefix(arg, "npc")) {
    if (!argument || !*argument || argument[0] == '?') {
      cmd_syntax(ch, do_name, "npc <keyword or id#> 	- edit a specific npc",
                 "npc create	 	- create an npc", "npc list		- list npcs", 0);
      return;
    }
    Character *vch;
  ...
  if (!str_prefix(arg, "object")) {
    if (!argument || !*argument || argument[0] == '?') {
      cmd_syntax(ch, do_name, "object <keyword or id#> - edit a specific object",
                 "object create	 	 - create an object", "object list		 - list objects", 0);
      return;
    }
    Object *obj;
  ...
  if (!str_prefix(arg, "area")) {
    if (!argument || !*argument || argument[0] == '?') {
      cmd_syntax(ch, do_name, "area <keyword or id#> 	- edit a specific area",
                 "area create	 	- create an area", "area list		- list areas", 0);
      return;
    }
    Area *area;
...
DOFUN(sockets) {
  ...
  for (Client *c = first_client; c; c = c->next) {
    xwritelnf(buf, "~W%s~x - ~C%s~x (~G%s~x) ~B%s~x", getip(c), c->host, c->termType,
              c->account->playing ? c->account->playing->name : "Unknown");
  }
...
DOFUN(import) {
  if (nullstr(argument)) {
    cmd_syntax(ch, do_name, "rom <filename>	- import a rom area file",
  ...
  if (!str_cmp(arg, "rom")) {
    ...
    if (fp == 0) {
      xwritelnf(ch, "Could not open %s for reading.", argument);
      return;
    }
...
DOFUN(db) {
  if (nullstr(argument)) {
    cmd_syntax(ch, do_name, "save [type]	- saves to the database", 0);
    return;
  }
  char arg[ARG_SIZ];
  argument = one_argument(argument, arg);
