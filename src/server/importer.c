...
void import_finalize_exits(Room *room) {
  for (int e = 0; e < MAX_DIR; e++) {
    ...
    if (!toRoom) {
      log_bug("exit (%" PRId64 ") with no room %" PRId64, room->exits[e]->id, id);
...
void npc_import_cleanup(void *data) { destroy_char((Character *)data); }

void obj_import_cleanup(void *data) { destroy_object((Object *)data); }
...
void import_commit(bool load) {
  ...
  for (TmpReset *tmp_next, *tmp = temp_resets; tmp; tmp = tmp_next) {
    ...
    for (TmpReset *sub_next = 0, *sub = tmp->subresets; sub; sub = sub_next) {
      sub_next = sub->next;
      free_mem(sub);
    }
    free_mem(tmp);
...
void server_import(const char *db_path, const char *file) {
  ...
  if (!str_suffix(".lst", file)) {
    if (!(success = import_rom_area_list(dirname((char *)file), fp))) {
...
void rom_act_flag_convert(Flag *flags, long bits) {
  static const struct {
    long rombit;
    int bit;
  } actconvert[] = {{B, NPC_SENTINEL},  // ACT_SENTINEL
                    {C, NPC_SCAVENGER},
                    {F, NPC_AGGRESSIVE},
                    {G, NPC_STAY_AREA},
                    {H, NPC_WIMPY}};
...
void rom_resists_flag_convert(Character *ch, long bits, int mod) {
  static const struct {
    int resist;
    long rombit;
  } resistconv[] = {{DAM_BASH, E},   {DAM_PIERCE, F},  {DAM_SLASH, G},  {DAM_FIRE, H}, {DAM_COLD, I},
                    {DAM_ACID, J},   {DAM_POISON, K},  {DAM_SHADOW, L}, {DAM_HOLY, M}, {DAM_ENERGY, N},
                    {DAM_MENTAL, O}, {DAM_DISEASE, P}, {DAM_LIGHT, S}};
...
float rom_convert_size(const char *word) {
  static const struct {
    float size;
    const char *name;
  } sizetable[] = {{1.8f, "tiny"},  {2.7f, "small"}, {4.8f, "medium"},
                   {6.7f, "large"}, {8.8f, "huge1"}, {10.0f, "giant"}};
...
Flag *rom_convert_weapon_flags(long bits) {
  static const struct {
    int rombit;
    int bit;
  } convert[] = {{A, WEAPON_FLAMING}, {B, WEAPON_FROST},     {C, WEAPON_VAMPIRIC}, {D, WEAPON_SHARP},
                 {E, WEAPON_VORPAL},  {F, WEAPON_TWO_HANDS}, {G, WEAPON_SHOCKING}, {H, WEAPON_POISON}};
  Flag *flag = new_flag();
...
Flag *rom_convert_container_flags(long bits) {
  static const struct {
    ...
    int bit;
      ...
      {A, CONT_CLOSEABLE}, {B, CONT_PICKPROOF}, {C, CONT_CLOSED}, {D, CONT_LOCKED}, {E, CONT_PUT_ON},
  };
  Flag *flag = new_flag();
...
void rom_load_obj_values(Object *pObjIndex, int item_type, FILE *fp) {
  switch (item_type) {
     ...
     */
    case 5:
      vset_long(&pObjIndex->value[0], value_lookup(weapon_types, fread_word(fp)));
      vset_long(&pObjIndex->value[1], fread_number(fp));
      vset_long(&pObjIndex->value[2], fread_number(fp));
      vset_long(&pObjIndex->value[3], value_lookup(dam_types, fread_word(fp)));
      vset_ptr(&pObjIndex->value[4], rom_convert_weapon_flags(fread_flag(fp)));
      break;
    // case ITEM_CONTAINER:
    case 15:
      vset_long(&pObjIndex->value[0], fread_number(fp));
      vset_ptr(&pObjIndex->value[1], rom_convert_container_flags(fread_flag(fp)));
      vset_long(&pObjIndex->value[2], fread_number(fp));
...
void rom_convert_obj_flags(Flag *flags, long flag) {
  static const struct {
    int bit;
    long rombit;
  } convert[] = {{ITEM_GLOW, (A)},      {ITEM_HUM, (B)},       {ITEM_DARK, (C)},         {ITEM_EVIL, (E)},
                 {ITEM_INVIS, (F)},     {ITEM_MAGIC, (G)},     {ITEM_NODROP, (H)},       {ITEM_BLESS, (I)},
                 {ITEM_ANTI_GOOD, (J)}, {ITEM_ANTI_EVIL, (K)}, {ITEM_ANTI_NEUTRAL, (L)}, {ITEM_NOREMOVE, (M)},
                 {ITEM_INVENTORY, (N)}, {ITEM_NOPURGE, (O)},   {ITEM_ROT_DEATH, (P)},    {ITEM_BURN_PROOF, (Y)}};
...
Flag *rom_convert_affect_flags(long bits) {
  static const struct {
    int bit;
    long rombit;
  } convert[] = {{AFF_BLIND, (A)},        {AFF_INVISIBLE, (B)},     {AFF_DETECT_EVIL, (C)},   {AFF_DETECT_INVIS, (D)},
                 {AFF_DETECT_MAGIC, (E)}, {AFF_DETECT_HIDDEN, (F)}, {AFF_DETECT_GOOD, (G)},   {AFF_SANCTUARY, (H)},
                 {AFF_FAERIE_FIRE, (I)},  {AFF_INFRARED, (J)},      {AFF_CURSE, (K)},         {AFF_POISON, (M)},
                 {AFF_PROTECT_EVIL, (N)}, {AFF_PROTECT_GOOD, (O)},  {AFF_SNEAK, (P)},         {AFF_HIDE, (Q)},
                 {AFF_SLEEP, (R)},        {AFF_CHARM, (S)},         {AFF_FLYING, (T)},        {AFF_PASS_DOOR, (U)},
                 {AFF_HASTE, (V)},        {AFF_CALM, (W)},          {AFF_PLAGUE, (X)},        {AFF_WEAKEN, (Y)},
                 {AFF_DARK_VISION, (Z)},  {AFF_BERSERK, (aa)},      {AFF_REGENERATION, (cc)}, {AFF_SLOW, (dd)}};
  Flag *flags = new_flag();
...
AffectCallback *convert_affect_location(int location) {
  static const struct {
    ...
    int location;
  } convert[] = {{&affect_apply_str, 1},  {&affect_apply_dex, 2},  {&affect_apply_int, 3},   {&affect_apply_wis, 4},
                 {&affect_apply_con, 5},  {&affect_apply_sex, 6},  {&affect_apply_level, 8}, {&affect_apply_mana, 12},
                 {&affect_apply_hit, 13}, {&affect_apply_move, 14}};
...
bool import_rom_objects(FILE *fp) {
  ...
  for (;;) {
    ...
    pObjIndex->type = (object_type)value_lookup(object_types, item_type);
    rom_convert_obj_flags(pObjIndex->flags, fread_flag(fp));
    pObjIndex->wearFlags = import_rom_convert_wear_flags(fread_flag(fp));
    rom_load_obj_values(pObjIndex, lookup_rom_obj_type(item_type), fp);
...
void rom_convert_room_flags(Flag *flags, long bits) {
  static const struct {
    int bit;
    long rombit;
  } convert[] = {{ROOM_SAFE, (K)}, {ROOM_NO_RECALL, (N)}};
  ...
  for (;;) {
    ...
    for (;;) {
      ...
      else if (letter == 'C') /* clan */
      {
        ...
        if (door < 0 || door > 5) {
          log_bug("%s bad door number.", pRoomIndex->name);
        ...
        switch (locks) {
          case 0:
            break;
          case 1:
            set_bit(set_bit(pexit->flags, EXIT_ISDOOR), EXIT_CLOSED);
            break;
          case 2:
            set_bit(set_bit(set_bit(pexit->flags, EXIT_ISDOOR), EXIT_CLOSED), EXIT_LOCKED);
            break;
          case 3:
        ...
        log_bug("room %s has flag not 'DES'.", pRoomIndex->name);
        destroy_room(pRoomIndex);
        return false;
      }
    }
...
int convert_rom_wear_loc(int val) {
  int table[] = {WEAR_LIGHT, WEAR_FINGER,  WEAR_FINGER_2, WEAR_NECK, WEAR_NECK,   WEAR_TORSO, WEAR_HEAD,
                 WEAR_LEGS,  WEAR_FEET,    WEAR_HANDS,    WEAR_ARMS, WEAR_SHIELD, WEAR_ABOUT, WEAR_WAIST,
                 WEAR_WRIST, WEAR_WRIST_2, WEAR_WIELD,    WEAR_HOLD, WEAR_FLOAT};
    ...
    switch (letter) {
      ...
      case 'P':
        ...
        if ((pRoomIndex = hm_get(room_indexes, iLastObj))) {
          if (last_reset == 0 || (last_reset->type != 'O' && last_reset->type != 'G' && last_reset->type != 'E')) {
      ...
      case 'G':
        ...
        if (hm_get(obj_indexes, pReset->arg1) == 0) {
          log_bug("bad object for G reset %ld.", pReset->arg1);
          free_mem(pReset);
          break;
        }

        if ((pRoomIndex = hm_get(room_indexes, iLastRoom))) {
          if (last_reset == 0 || (last_reset->type != 'M')) {
      ...
      case 'D':
        ...
        if ((pRoomIndex = hm_get(room_indexes, pReset->arg1)) == 0) {
          log_bug("bad room for reset");
          free_mem(pReset);
          break;
        }

        if (pReset->arg2 < 0 || pReset->arg2 > 5 || !pRoomIndex || !(pexit = pRoomIndex->exits[pReset->arg2]) ||
            !is_set(pexit->flags, EXIT_ISDOOR)) {
          log_bug("exit %ld not door, room %ld.", pReset->arg2, pReset->arg1);
          free_mem(pReset);
          break;
        }
