...
void reset_room(Room *room) {
  ...
  while (str && *str != 0) {
    ...
    if (!str_cmp("npc", tmp)) {
      if ((tmp = strtok(0, delims)) == 0 || (id = atol(tmp)) == 0 || (lastNPC = load_npc(id)) == 0) {
        log_error("npc reset - could not load npc %" PRId64 " (room %" PRId64 ")", id, room->id);
        continue;
      }
      int max = 1;
      ...
      for (Character *rch = room->characters; rch; rch = rch->next_in_room)
    ...
    if (!str_cmp("obj", tmp)) {
      if ((tmp = strtok(0, delims)) == 0 || (id = atol(tmp)) == 0 || (lastOBJ = load_object(id)) == 0) {
        log_error("obj reset - could not load obj %" PRId64 " (room %" PRId64 ")", id, room->id);
        continue;
      }
      int max = 1;
      ...
      for (Object *obj = room->objects; obj; obj = obj->next_content)
    ...
    if (!str_cmp("give", tmp)) {
      if (lastNPC == 0) {
        log_error("give reset - no last npc (room %" PRId64 ")", room->id);
        continue;
      }

      if ((tmp = strtok(0, delims)) == 0 || (id = atol(tmp)) == 0 || (lastOBJ = load_object(id)) == 0) {
        log_error("give reset - could not load obj %" PRId64 " (room %" PRId64 ")", id, room->id);
        continue;
      }
      int max = 1;
      ...
      for (Object *obj = lastNPC->carrying; obj; obj = obj->next_content)
    ...
    if (!str_cmp("equip", tmp)) {
      if (lastNPC == 0) {
        log_error("equip reset - no last npc (room %" PRId64 ")", room->id);
        continue;
      }

      if ((tmp = strtok(0, delims)) == 0 || (id = atol(tmp)) == 0 || (lastOBJ = load_object(id)) == 0) {
        log_error("equip_reset - could not load obj %" PRId64 " (room %" PRId64 ")", id, room->id);
        continue;
      }
      obj_to_char(lastOBJ, lastNPC);
    ...
    if (!str_cmp("put", tmp)) {
      if (lastOBJ == 0) {
        log_error("put reset - no last obj (room %" PRId64 ")", room->id);
        continue;
      }
      Object *obj;

      if ((tmp = strtok(0, delims)) == 0 || (id = atol(tmp)) == 0 || (obj = load_object(id)) == 0) {
        log_error("put reset - could not load obj %" PRId64 " ( room %" PRId64 ")", id, room->id);
        continue;
      }
...
long mana_gain(Character *ch) {
  ...
  if (!ch->pc) {
    ...
    gain = (getCurrStat(ch, STAT_WIS) + getCurrStat(ch, STAT_INT) + ch->level);
    // gain += bonus(ch) * 2;
    /*
     * number = number_percent(); if (number <
   ...
   *
   * if (ch->on != NULL && ch->on->item_type == ITEM_FURNITURE) gain =
...
void gain_condition(Character *ch, int iCond, int value) {
  ...
  if (iCond == COND_THIRST && value < 0 && ch->inRoom && ch->inRoom->sector == SECT_DESERT) {
    value *= 2;
  ...
  ch->pc->condition[iCond] = URANGE(iCond == COND_DRUNK ? 0 : -50, condition + value, 75);
...
void character_update() {
  for (Character *ch = first_character; ch != 0; ch = ch->next) {
    if (ch->pc) {
      ...
      if (!is_set(ch->pc->account->flags, PLR_TICKS_OFF) && conn->editing == 0 && client_is_playing(conn)) {
        char *const mesg[] = {"TICK", "TOCK"};

        if (number_range(0, 100) < 50)
          xwritelnf(ch, "~?[%s] %s!~x", str_time(-1, ch->pc->account->timezone, "%I:%M:%S"),
                    mesg[number_range(0, 10) < 5 ? 0 : 1]);

        else
          xwritelnf(ch, "~?%s!~x", mesg[number_range(0, 10) < 5 ? 0 : 1]);
      }
    }
    ...
    for (Affect *paf_next, *paf = ch->affects; paf; paf = paf_next) {
      ...
      if (paf->duration > 0) {
        ...
        if (!paf_next || paf_next->from != paf->from || paf_next->duration > 0) {
          if (valid_skill(paf->from) && !nullstr(skill_table[paf->from].msgOff)) {
            xwriteln(ch, skill_table[paf->from].msgOff);
          }
        }
