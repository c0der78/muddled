...
bool channel_viewable( const Character * ch, const Character * victim,
					   const Channel * chan )
{
...
const char *say_verb( const char *word, Character * ch,
					  Character * viewer, int S )
{
...
#define sm(a, b, c) return S == 0 ? a : S == 1 ? b : c
...

	{
		...
		/*else if (ch && IS_DEAD(ch))
		   sm("wail", "wails", "wailed"); */
	}
	...
	for ( pch = first_player; pch; pch = pch->next_player )
	{
		if ( pch == ch || pch == victim || channel_viewable( ch, pch, chan ) )
		{
			/*           const char *string = get_chan_soc_string(ch, victim, pch, obj, soc);
			   ...
			   if (fRand)
			   msg = FORMATF("%s %s (%s){x", type, string, soc->getName());
			   else
			   msg = FORMATF("%s %s{x", type, string);

			   perform_act(msg, ch, obj, victim, false, vch, true, true);
			   }
			   update_last_data(ch, vch, chan, perform_act_string(string, ch, obj, victim, false), CHANNEL_SOCIAL); */
		}
	}
}
...
int interpret_channel( Character * ch, int gcn, const char *argument )
{
	...
	else if ( command[0] == '@' )
	{
		...
		writelnf( ch, "%s %s . o O ( %s ){x", format, ch->name, argument );
	}
	...
	else if ( is_exact_name( command, "-h -help" ) && nullstr( argument ) )
	{
		...
		if ( chan->bit )
			writelnf( ch,
					  ...
					  chan->name );
		writelnf( ch, "      : %s -hist              - display channel history",
				  chan->name );
		writelnf( ch,
	...
	else
	{
		...
		for ( Character * pch = first_player; pch != 0; pch = pch->next_player )
		{
			...
			{
				case CHANNEL_NORMAL:
					writelnf( pch, "%s %s %s '%s'~x", format, chview( ch, pch ),
							  say_verb( argument, ch, pch, 1 ), argument );
					break;
				default:
					break;
			}
