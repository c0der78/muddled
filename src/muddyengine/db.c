...
int field_map_int(const field_map * table)
{
	return *((int *)table->value);
}

float field_map_float(const field_map * table)
{
	return *((float *)table->value);
}

Flag *field_map_flag(const field_map * table)
{
	return *((Flag **) table->value);
}

double field_map_double(const field_map * table)
{
	return *((double *)table->value);
}

const char *field_map_str(const field_map * table)
{
...
int db_load(field_map * table, const char *tableName, const char *constraints, ...)
{
	char buf[OUT_SIZ] = {0};
	...
	if (sql_select_query(table, tableName, buf) != SQL_DONE) {
	...
	case SQL_LOOKUP:
		{
			...
			if (name == 0)
				return sql_bind_null(stmt, index);
			return sql_bind_text(stmt, index, name, strlen(name),
					     0);
		}
	case SQL_CUSTOM:
		{
			custom_sql func = (custom_sql) (table->arg1);
			assert(func != 0);
...
int sql_select_query(field_map * table, const char *tablename, const char *constraints)
{
	char buf[OUT_SIZ] = { 0 };
	char columns[OUT_SIZ] = { 0 };
	...
	int len = sprintf(buf, "select %s,%s from %s %s",
	...
	for (err = sql_step(stmt); err != SQL_DONE; err = sql_step(stmt)) {
		for (int i = 0; table[i].name != 0; i++) {
			...
			case SQL_FLOAT:
				*((float *)table->value) =
				    sql_column_float(stmt, i);
				break;
			case SQL_DOUBLE:
				*((double *)table->value) =
				    sql_column_double(stmt, i);
				break;
			case SQL_FLAG:
				parse_flags(*((Flag **) table->value),
 ...
 * { for ( int i = 0; table[i].name != 0; i++ ) { strcat( names,
 * table[i].name ); strcat( names, "," ); }
 * 
 * if ( names[0] != 0 ) names[strlen( names ) - 1] = 0;
 * 
 * for ( int i = 0; table[i].name != 0; i++ ) { set_db_values( &table[i],
 * values ); }
 * 
 * if ( values[0] != 0 ) values[strlen( values ) - 1] = 0; }
 * 
 * void build_update_values( field_map *table, char *values ) { for ( int i 
 * = 0; table[i].name != 0; i++ ) { strcat( values, table[i].name );
 * strcat( values, "=" );
 * 
