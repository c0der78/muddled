	 ...
	 const char *color, int lm, int rm, const char *value)
{
    ...
    grid_set(grid, grid->x, grid->y, align, colspan, border, color, lm,
	     rm);
...
void grid_print(Grid * grid, Character * ch)
{
	...
	for (int j = 0; j < grid->rows[i].columns;
		...
		writef(buf, "%s%c~x", border->color, border->vertical);
		...
		int loss =
		    (grid->width - (row->columnsize * row->columns));
	    ...
	    write(buf,
		  align_string(cell->align, colwidth, cell->color,
			       ...
			       &border->horizontal : 0, cell->value));
