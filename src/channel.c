...
const Lookup channel_flags[] = {{"chat", CHANNEL_CHAT}, {"admin", CHANNEL_ADMIN}, {0, 0}};

const Channel channel_table[] = {
    {&gcn_chat, CHANNEL_CHAT, "chat", "A global channel for open communication.", "~m[~MChat~m]", "~M", channel_global},
    {&gcn_admin, CHANNEL_ADMIN, "admin", "A channel for administrators.", "~c[~CAdmin~c]", "~C", channel_admin},
    {0, 0, 0}};
...
bool channel_viewable(const Character *ch, const Character *victim, const Channel *chan) {
  switch (chan->type) {
    case channel_global:
      return true;
    case channel_admin:
...
const char *say_verb(const char *word, Character *ch, Character *viewer, int S) {
  size_t len = strlen(word), i, j;

#define sm(a, b, c) return S == 0 ? a : S == 1 ? b : c

  if ((viewer && viewer->pc && is_set(viewer->flags, PLR_NOEMOTEVERBS)) || len < 3) {
    sm("say", "says", "said");
  }
...
const char *get_chan_soc_string(Character *ch, Character *victim, Character *vch, Object *obj, Social *soc) {
  if (!victim && !obj) {
    if (ch == vch) {
...
void channel_social(Character *ch, Character *victim, Object *obj, Social *soc, const Channel *chan) {
  Character *pch;
  const char *type;
  type = format_channel(chan, ch);

  for (pch = first_player; pch; pch = pch->next_player) {
    if (pch == ch || pch == victim || channel_viewable(ch, pch, chan)) {
      const char *string = get_chan_soc_string(ch, victim, pch, obj, soc);
      char msg[BUFSIZ + 1] = {0};
...
int interpret_channel(Character *ch, int gcn, const char *argument) {
  ...
  if (command[0] == '+') {
    ...
    if (nullstr(command)) {
      xwritelnf(ch, "{W%s + <social> is used for channel based socials.{x", chan->name);
      return 1;
    }
    ...
    if (nullstr(argx)) {
      ...
      if (!nullstr(soc->othersObjFound) && ch->inRoom &&
          ((obj = get_obj_list(ch, argx, ch->inRoom->objects)) || (obj = get_obj_carry(ch, argx, ch)))) {
    ...
    xwritelnf(ch, "%s %s . o O ( %s ){x", format, ch->name, argument);
  }

  else if (is_exact_name(command, "-who -w -wholist") && nullstr(argument)) {
    ...
    xwritelnf(ch, "Syntax: %s <message>          - send a message", chan->name);

    if (chan->bit) xwritelnf(ch, "      : %s                    - toggle channel on/off", chan->name);
    xwritelnf(ch, "      : %s -hist              - display channel history", chan->name);
    xwritelnf(ch, "      : %s -who               - display who is on channel", chan->name);
    xwritelnf(ch, "      : %s ! <emote>          - send an emote over channel", chan->name);
    xwritelnf(ch, "      : %s + <social> [args]  - do a social over channel", chan->name);
    xwritelnf(ch, "      : %s @ <message>        - enclose a message in 'thought bubbles'", chan->name);
    xwritelnf(ch, "      : %s -help              - this message", chan->name);
    return 1;
  }

  else {
    xwritelnf(ch, "%s You %s '%s'~x", format, say_verb(argument, ch, 0, 0), argument);

    for (Character *pch = first_player; pch != 0; pch = pch->next_player) {
      ...
      switch (arg_type) {
        case CHANNEL_NORMAL:
          xwritelnf(pch, "%s %s %s '%s'~x", format, chview(ch, pch), say_verb(argument, ch, pch, 1), argument);
          break;
        default:
          break;
      }
