...
const Lookup plr_flags[] = {
    {"ticksoff", PLR_TICKS_OFF}, {"brief", PLR_BRIEF}, {"hints", PLR_HINTS}, {"automapoff", PLR_AUTOMAP_OFF}, {0, 0}};
...
Player *new_player(Connection *conn) {
  ...
  p->battlePrompt = str_dup("[~R%h~x/~R%H~rhp ~M%m~x/~M%M~mm ~B%v/~B%V~bmv~x] %E");
  p->conn = conn;
...
void load_player_columns(Account *acc, Character *ch, sql_stmt *stmt) {
  ...
  for (i = 0; i < cols; i++) {
    ...
    if (load_char_column(ch, stmt, colname, i)) {
      ...
      if (acc && acc->id != sql_column_int(stmt, i)) log_error("sql returned invalid account for player");
    }
      ...
      convert_explored_rle(ch->pc->explored, sql_column_str(stmt, i));
    }

    else if (!str_cmp(colname, "channels")) {
      parse_flags(ch->pc->channels, sql_column_str(stmt, i), channel_flags);
    }
...
bool is_drunk(Character *ch) { return ch->pc->condition[COND_DRUNK] > 10; }
