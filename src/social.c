...
int interpret_social(Character *ch, const char *command, const char *argument) {
  ...
  for (soc = first_social; soc != 0; soc = soc->next) {
    if (UPPER(command[0]) == UPPER(soc->name[0]) && !str_prefix(command, soc->name)) {
      break;
    }
  }
  ...
  if (ch->position > soc->minPosition) {
    xwritelnf(ch, "You can't do that while your %s.", position_table[ch->position].name);
    return 1;
  }
  ...
  if (arg[0] == 0) {
    act_pos(TO_ROOM, soc->minPosition, ch, NULL, victim, soc->othersNoArg);
    ...
    act_pos(TO_NOTVICT, soc->minPosition, ch, NULL, victim, soc->othersFound);
    ...
    if (!ch->npc && victim->npc && victim->position < POS_SLEEPING) {
      switch (number_bits(4)) {
        ...
        case 7:
        case 8:
          act_pos(TO_NOTVICT, soc->minPosition, victim, NULL, ch, soc->othersFound);
          act_pos(TO_CHAR, soc->minPosition, victim, NULL, ch, soc->charFound);
          act_pos(TO_VICT, soc->minPosition, victim, NULL, ch, soc->victFound);
          break;
        case 9:
        case 10:
        case 11:
...
int save_social(Social *soc) {
  field_map social_values[] = {{"name", &soc->name, SQL_TEXT},
