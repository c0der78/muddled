...
const Lookup sex_table[] = {
    {"neutral", SEX_NEUTRAL}, {"male", SEX_MALE}, {"female", SEX_FEMALE}, {"hermaphrodite", SEX_HERMAPHRODITE}, {0, 0}};

const Lookup position_table[] = {
    {"fighting", POS_FIGHTING},       {"standing", POS_STANDING}, {"sitting", POS_SITTING},
    {"resting", POS_RESTING},         {"stunned", POS_STUNNED},   {"incapacitated", POS_INCAPICATED},
    {"mortally wounded", POS_MORTAL}, {"dead", POS_DEAD},         {0, 0}};
...
int load_char_objs(Character *ch) {
  ...
  sql_stmt *stmt;
  int total = 0;
  int len = sprintf(buf, "select * from char_objects where carriedById=%" PRId64, ch->id);
...
int load_char_column(Character *ch, sql_stmt *stmt, const char *colname, int i) {
...
int delete_character(Character *ch) {
  char buf[BUF_SIZ];
  sprintf(buf, "delete from character where characterId=%" PRId64, ch->id);
...
int save_char_objs(Character *ch) {
  ...
  for (obj = ch->carrying; obj; obj = obj->next_content) {
    len = sprintf(buf, "select objectId from char_objects where carriedById=%" PRId64 " and objectId=%" PRId64, ch->id,
                  obj->id);
  ...
  len = sprintf(buf, "select * from char_objects where carriedById=%" PRId64, ch->id);
  ...
  while (sql_step(stmt) != SQL_DONE) {
    ...
    if (!obj) {
      sprintf(buf, "delete from object where carriedById=%" PRId64 " and objectId=%" PRId64, ch->id, id);
...
int save_character(Character *ch, const Lookup *flag_table) {
  ...
  field_map char_values[] = {{"version", &CharSaveVersion, SQL_INT},
  ...
  if (ch->id == 0) {
    ...
    if (sql_update_query(char_values, "character", ch->id) != SQL_OK) {
...
int load_char_affects(Character *ch) {
  ...
  sql_stmt *stmt;
  int total = 0;
  int len = sprintf(buf, "select * from char_affect where characterId=%" PRId64, ch->id);
...
int save_char_affects(Character *ch) {
  ...
  for (aff = ch->affects; aff; aff = aff->next) {
    len = sprintf(buf, "select charAffectId from char_affect where characterId=%" PRId64 " and affectId=%" PRId64,
                  ch->id, aff->id);
    ...
    field_map char_affect[] = {{"characterId", &ch->id, SQL_INT}, {"affectId", &aff->id, SQL_INT}, {0}};

    if (!update) {
      if (sql_insert_query(char_affect, "char_affect") != SQL_OK) {
      ...
      if (sql_update_query(char_affect, "char_affect", id) != SQL_OK) {
        log_data("could not update character");
        return 0;
      }
    }
  ...
  // try and cleanup old objects
  len = sprintf(buf, "select charAffectId from char_affect where characterId=%" PRId64, ch->id);
  ...
  while (sql_step(stmt) != SQL_DONE) {
    ...
    if (!aff) {
      sprintf(buf, "delete from char_affect where characterId=%" PRId64 " and affectId=%" PRId64, ch->id, id);
...
short scrwidth(const Character *ch) { return !ch->pc || !ch->pc->conn ? 80 : ch->pc->conn->scrWidth; }

short scrheight(const Character *ch) { return !ch->pc || !ch->pc->conn ? 24 : ch->pc->conn->scrHeight; }

bool is_immortal(const Character *ch) { return (ch->level >= LEVEL_IMMORTAL); }

bool is_implementor(const Character *ch) { return (ch->level >= MAX_LEVEL); }

bool is_playing(const Character *ch) { return ch->pc && ch->pc->conn && ch->pc->conn->is_playing(ch->pc->conn); }

bool is_player(const Character *ch) { return ch->pc && !is_immortal(ch); }
...
Character *get_char_world(Character *ch, const char *argument) {
  ...
  for (wch = first_character; wch != NULL; wch = wch->next) {
    if (wch->inRoom == NULL || !can_see(ch, wch) || !is_name(arg, wch->name)) {
      continue;
    }
