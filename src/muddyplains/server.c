...
void initialize_server()
{
...
#if defined SO_DONTLINGER && !defined SYSV
	{
	    ...
	    if (setsockopt
		(server_socket, SOL_SOCKET, SO_DONTLINGER, (char *) &ld,
		 sizeof(ld)) < 0) {
	...
	if (fp == 0) {
	    log_error("Could not open file %s for reading", REBOOT_FILE);
	} else {
	    log_trace("parsing reboot file");
	    if (fgets(buf, sizeof(buf), fp) != NULL) {
		if (sscanf(buf, "%ld\n", &startup_time) != 1) {
		    log_error("reboot: could not get startup time");
		}
	    }
		...
		if (getpeername
		    (conn->socket,
		     (struct sockaddr *) &conn->addr, &size) < 0) {
		    log_error("getpeername");
		    conn->host = str_dup("(unknown)");
		} else {
		    from =
			gethostbyaddr((char *) &conn->addr.sin_addr,
				      sizeof(conn->addr.sin_addr),
				      AF_INET);
		    conn->host =
			str_dup(from ? from->h_name : getip(conn));
		...
		for (AccountPlayer * ch = conn->account->players;
		     ch != 0; ch = ch->next) {
		    if (!str_cmp(ch->name, name)) {
			log_info("reloaded %s", name);
			conn->account->playing =
			    load_player_by_id((Connection
					       *) conn, ch->charId);
			break;
		    }
		}
...
void game_loop(int control)
{
	 ...
	 */
	{
	    ...
	    secDelta = (suseconds_t) (last_time.tv_sec - now_time.tv_sec);
...
void reboot_server()
{
    ...
    execl(exec_path, exec_path, arg1, arg2, (char *) NULL);
