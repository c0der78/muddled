...
void reset_room( Room * room )
{
	...
	while ( str && *str != 0 )
	{
		...
		if ( !str_cmp( "npc", tmp ) )
		{
			...
			{
				log_error( "npc reset - could not load npc %"PRId64" (room %"PRId64")", id,
						   room->id );
				continue;
			}
			int max = 1;
		...
		if ( !str_cmp( "obj", tmp ) )
		{
			...
			{
				log_error( "obj reset - could not load obj %"PRId64" (room %"PRId64")", id,
						   room->id );
				continue;
			}
		...
		if ( !str_cmp( "give", tmp ) )
		{
			...
			if ( ( tmp = strtok( 0, delims ) ) == 0
				 ...
				 || ( lastOBJ = load_object( id ) ) == 0 )
			{
				log_error( "give reset - could not load obj %"PRId64" (room %"PRId64")", id,
						   room->id );
				continue;
			}
		...
		if ( !str_cmp( "equip", tmp ) )
		{
			...
			if ( ( tmp = strtok( 0, delims ) ) == 0
				 ...
				 || ( lastOBJ = load_object( id ) ) == 0 )
			{
				log_error( "equip_reset - could not load obj %"PRId64" (room %"PRId64")", id,
						   room->id );
				continue;
			}
		...
		if ( !str_cmp( "put", tmp ) )
		{
			...
			{
				log_error( "put reset - could not load obj %"PRId64" ( room %"PRId64")", id,
						   room->id );
				continue;
			}
