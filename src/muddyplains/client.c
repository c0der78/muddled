...
static void
vtitle_to_client(Client * conn, const char *title, va_list args)
{
...
bool parse_color_codes(Client * conn)
{
	...
	if ((result - output) >= OUT_SIZ - 32) {
	    ...
	    *result++ = 0;
	    ...
	    written =
		write_to_socket(conn->socket, output, result - output);
...
void client_account_menu(Client * conn, const char *argument)
{
	...
	writeln(conn, "~CAre you sure you want to delete your account?~x");
