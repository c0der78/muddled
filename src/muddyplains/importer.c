...
void import_finalize_reset( TmpReset * res, char *buf, int tab )
{
	...
	switch ( res->type )
	{
		...
		case 'E':
			...
			/*
			   lookup_name( wear_flags,
			   wear_type_to_flag( res->arg2 ) ) ); */
...
void server_import( const char *file )
{
	...
	if ( !str_suffix( ".lst", file ) )
	{
		if ( !
			 ( success =
			   import_rom_area_list( dirname( ( char * ) file ), fp ) ) )
...
bool import_rom_area( FILE * fp )
{
	...
	fread_string( fp );			// credits
	fread_number( fp );			// lvnum
	fread_number( fp );			// uvnum
...
bool import_rom_objects( FILE * fp )
{
	...
	for ( ;; )
	{
		...
		pObjIndex->type = (wear_type) value_lookup( object_types, item_type );
		...
		for ( ;; )
		{
			...
			else if ( letter == 'D' )
			{
				...
				switch ( locks )
				{
					...
					case 1:
						set_bit( set_bit
								 ( pexit->flags, EXIT_ISDOOR ), EXIT_CLOSED );
						break;
					case 2:
						set_bit( set_bit
								 ( set_bit
								   ( pexit->flags,
									 EXIT_ISDOOR ),
								   EXIT_CLOSED ), EXIT_LOCKED );
						break;
					case 3:
						//pexit->flags = EX_ISDOOR | EX_CLOSED | EX_LOCKED | EX_NOPASS; break;
						set_bit( set_bit
								 ( set_bit
								   ( pexit->flags,
									 EXIT_ISDOOR ),
						...
						break;
					case 4:
						//pexit->flags = EX_ISDOOR | EX_CLOSED | EX_LOCKED | EX_PICKPROOF; break;
						set_bit( set_bit
								 ( set_bit
								   ( pexit->flags,
									 EXIT_ISDOOR ),
						...
						break;
					case 5:
						//pexit->flags = EX_ISDOOR | EX_CLOSED | EX_LOCKED | EX_PICKPROOF | EX_NOPASS; break;
						set_bit( set_bit
								 ( set_bit
								   ( pexit->flags,
									 EXIT_ISDOOR ),
		...
		switch ( letter )
		{
			...
			case 'D':
				...
				switch ( pReset->arg3 )
				{
					...
					case 0:
						break;
					case 1:	/*SET_BIT( pexit->rs_flags, EX_ISDOOR|EX_CLOSED ); */
						break;
					case 2:	/*SET_BIT( pexit->rs_flags, EX_ISDOOR|EX_CLOSED|EX_LOCKED ); */
						break;
					case 3:	/*SET_BIT( pexit->rs_flags, EX_ISDOOR|EX_CLOSED|EX_LOCKED|EX_NOPASS ); */
						break;
					case 4:	/*SET_BIT( pexit->rs_flags, EX_ISDOOR|EX_CLOSED|EX_LOCKED|EX_PICKPROOF ); */
						break;
					case 5:	/*SET_BIT( pexit->rs_flags, EX_ISDOOR|EX_CLOSED|EX_LOCKED|EX_NOPASS|EX_PICKPROOF ); */
						break;
				}
			...
			case 'R':
				...
				//if ( pReset->arg1 < max_room_index && ( pRoomIndex = room_indexes[ pReset->arg1 ] ) )
...
{
	char buf[OUT_SIZ * 4];		/* extra 2 bytes on the end for \0 and 1b slack */
	long i = 0;
	register char c;
	bool sFull = false;
