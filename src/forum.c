...
bool is_note_to(Character *ch, Note *note) {
  ...
  if (is_immortal(ch) && (is_exact_name("imm", note->toList) || is_exact_name("imms", note->toList) ||
                          is_exact_name("immortal", note->toList) || is_exact_name("god", note->toList) ||
                          is_exact_name("gods", note->toList) || is_exact_name("imp", note->toList) ||
                          is_exact_name("imps", note->toList) || is_exact_name("implementor", note->toList) ||
                          is_exact_name("implementors", note->toList) || is_exact_name("immortals", note->toList))) {
    return true;
  }
...
void show_note_to_char(Character *ch, Note *note, int num) {
  ...
  xwritelnf(buffer, "[~x%4d~x] ~Y%s~x: ~g%s~x", num, note->from, note->subject);
  xwritelnf(buffer, "~YDate~x:  %s", str_time(note->date, ch->pc->account->timezone, 0));
...
bool is_subscribed(Account *acc, int forum) { return !acc->forumData[forum].unsubscribed; }

void set_subscribed(Account *acc, int forum) { acc->forumData[forum].unsubscribed = false; }

void remove_subscribed(Account *acc, int forum) { acc->forumData[forum].unsubscribed = true; }
...
int unread_notes(Character *ch, Forum *forum) {
  ...
  for (Note *note = forum->notes; note; note = note->next)
    ...
    if (is_note_to(ch, note) && ((long)last_read < (long)note->date)) {
      count++;
    }
  return count;
}
...
void next_forum(Character *ch) {
  ...
  while ((i < max_forum) && (unread_notes(ch, &forum_table[i]) == FORUM_ERROR || !is_subscribed(ch->pc->account, i))) {
    i++;
  }
...
void show_forum(Character *ch, bool fAll) {
  ...
  for (i = 0; i < max_forum; i++) {
    ...
    found = true;
    xwritelnf(ch, "~W%2d~x> ~G%13s~x [~%c%4d~x] ~G%4d ~Y%s~x", count, forum_table[i].name, unread == 0 ? 'r' : 'R',
  ...
  xwriteln(ch, "");
  xwritef(ch, "Your current forum is ~W%s~x", ch->pc->account->forum->name);
  ...
  xwriteln(ch, "Use 'subscribe' to see what forums you are subscribed to.");
}

void make_note(const char *forum_name, const char *sender, const char *to, const char *subject, const int expire_days,
               const char *text) {
  ...
  while (sql_step(stmt) != SQL_DONE) {
    ...
    for (int i = 0; i < count; i++) {
      ...
      if (!str_cmp(colname, "noteId")) {
        ...
        if (forum->id != sql_column_int(stmt, i)) log_error("sql returned invalid note for forum");
      }
  ...
  while (sql_step(stmt) != SQL_DONE) {
    ...
    for (int i = 0; i < count; i++) {
      ...
      if (!str_cmp(colname, "forumId")) {
        ...
        forum_table[total].name = str_dup(sql_column_str(stmt, i));
      }

      else if (!str_cmp(colname, "summary")) {
        forum_table[total].description = str_dup(sql_column_str(stmt, i));
      }

      else if (!str_cmp(colname, "defaultTo")) {
        forum_table[total].defaultTo = str_dup(sql_column_str(stmt, i));
      }
...
int save_note(Forum *forum, Note *note) {
  field_map note_values[] = {{"text", &note->text, SQL_TEXT},      {"subject", &note->subject, SQL_TEXT},
                             {"date_stamp", &note->date, SQL_INT}, {"expire", &note->expire, SQL_INT},
                             {"toList", &note->toList, SQL_TEXT},  {"sender", &note->from, SQL_TEXT},
                             {"forumId", &forum->id, SQL_INT},     {0}};
