  ...
  while (sql_step(stmt) != SQL_DONE) {
    ...
    for (int i = 0; i < count; i++) {
      ...
      if (!str_cmp(colname, "name")) {
        class_table[total].name = str_dup(sql_column_str(stmt, i));
      }

      else if (!str_cmp(colname, "summary")) {
        class_table[total].description = str_dup(sql_column_str(stmt, i));
      }
...
bool is_valid_class(int index) { return index >= 0 && index < max_class; }
...
const char *class_short(Character *ch) {
  ...
  for (int *c = ch->classes; c && *c != -1; c++) {
    len += sprintf(&buf[i][len], "%.3s/", capitalize(class_table[*c].name));
  }
...
const char *class_who(Character *ch) {
  ...
  if (ch->classes[1] == -1) {
    sprintf(buf, "%.3s", capitalize(class_table[*ch->classes].name));
    return buf;
  }
  int count = 0;
  ...
  sprintf(buf, "%c+%d", toupper((int)class_table[*ch->classes].name[0]), count - 1);
  return buf;
}
