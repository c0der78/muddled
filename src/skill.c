...
int load_skill_levels(Skill *skill) {
  ...
  sql_stmt *stmt;
  int total = 0;
  int len = sprintf(buf, "select * from skill_level where skillId=%" PRId64, skill->id);
  ...
  while (sql_step(stmt) != SQL_DONE) {
    ...
    for (int i = 0; i < count; i++) {
      ...
      if (!str_cmp(colname, "skillId")) {
        if (sql_column_int(stmt, i) != skill->id) log_error("load_skill_levels: sql returned invalid skill");
      }
...
bool valid_skill(identifier_t sn) { return sn >= 0 && sn < max_skill; }
  ...
  while (sql_step(stmt) != SQL_DONE) {
    ...
    for (int i = 0; i < count; i++) {
      ...
      if (!str_cmp(colname, "name")) {
        skill_table[total].name = str_dup(sql_column_str(stmt, i));
      }
        ...
        skill_table[total].msgOff = str_dup(sql_column_str(stmt, i));
      }

      else if (!str_cmp(colname, "msgObj")) {
        skill_table[total].msgObj = str_dup(sql_column_str(stmt, i));
      }

      else if (!str_cmp(colname, "mana")) {
        skill_table[total].mana = sql_column_int(stmt, i);
      }

      else if (!str_cmp(colname, "wait")) {
        skill_table[total].wait = sql_column_int(stmt, i);
      }
        ...
        skill_table[total].damage = str_dup(sql_column_str(stmt, i));
      }

      else if (!str_cmp(colname, "flags")) {
        parse_flags(&skill_table[total].flags, sql_column_str(stmt, i), skill_flags);
      }

      else if (!str_cmp(colname, "minPos")) {
        skill_table[total].minPos = sql_column_int(stmt, i);
      }

      else if (!str_cmp(colname, "spell")) {
        skill_table[total].spellfun = spellfun_lookup(sql_column_str(stmt, i));
      }

      else if (!str_cmp(colname, "gsn")) {
        skill_table[total].pgsn = gsn_lookup(sql_column_str(stmt, i));
...
int save_skill(Skill *skill) {
  field_map skill_values[] = {{"name", &skill->name, SQL_TEXT},
  ...
  if (skill->id == 0) {
    ...
    if (sql_update_query(skill_values, "skill", skill->id) != SQL_OK) {
...
SPELL(magic_missile) { xwriteln(ch, "Got magic missile"); }
