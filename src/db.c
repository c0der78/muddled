...
sqlite3 *enginedb() { return engine_info.db; }
...
const char *sql_errmsg() { return sqlite3_errmsg(enginedb()); }
...
int db_last_insert_rowid() { return sqlite3_last_insert_rowid(enginedb()); }

int sql_exec(const char *buf) { return sqlite3_exec(enginedb(), buf, 0, 0, 0); }
...
int sql_query(const char *buf, int len, sql_stmt **stmt) { return sqlite3_prepare(enginedb(), buf, len, stmt, 0); }

int sql_col_int(sql_stmt *stmt, const char *column) { return sqlite3_column_int(stmt, sql_column_index(stmt, column)); }

const char *sql_column_str(sql_stmt *stmt, int column) { return (const char *)sqlite3_column_text(stmt, column); }
...
void fm_set_int(const field_map *field, int value) { *((int *)field->value) = value; }

int fm_int(const field_map *field) { return *((int *)field->value); }

void fm_set_float(const field_map *field, float value) { *((float *)field->value) = value; }

float fm_float(const field_map *field) { return *((float *)field->value); }

Flag *fm_flag(const field_map *field) { return *((Flag **)field->value); }

void fm_parse_flags(const field_map *field, const char *value) {
  parse_flags(fm_flag(field), value, (const Lookup *)field->arg1);
}

void fm_set_double(const field_map *field, double value) { *((double *)field->value) = value; }

double fm_double(const field_map *field) { return *((double *)field->value); }

void fm_set_str(const field_map *field, const char *value) { *((const char **)field->value) = str_dup(value); }

const char *fm_str(const field_map *field) { return *((const char **)field->value); }
...
int sql_bind_str(sql_stmt *stmt, int index, const char *str) { return sql_bind_text(stmt, index, str, strlen(str), 0); }
...
int sql_update_query(field_map *table, const char *tablename, identifier_t id) {
  ...
  sql_stmt *stmt;
  int len = sprintf(buf, "update %s set %s where %s=%" PRId64, tablename, params, idname, id);
...
int sql_select_query(field_map *table, const char *tablename, sql_callback_t callback, const char *constraints) {
  ...
  sql_stmt *stmt;
  int len = sprintf(buf, "select %s from %s %s", columns, tablename, constraints ? constraints : "");
  // log_trace("%s", buf);
