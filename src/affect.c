...
const Lookup affect_flags[] = {{"blindness", AFF_BLIND},
...
void affect_apply_str(Affect *paf, void *obj, bool fRemove) { affect_apply_stat(paf, obj, fRemove, STAT_STR); }

void affect_apply_dex(Affect *aff, void *obj, bool fRemove) { affect_apply_stat(aff, obj, fRemove, STAT_DEX); }

void affect_apply_int(Affect *paf, void *obj, bool fRemove) { affect_apply_stat(paf, obj, fRemove, STAT_INT); }

void affect_apply_wis(Affect *paf, void *obj, bool fRemove) { affect_apply_stat(paf, obj, fRemove, STAT_WIS); }

void affect_apply_con(Affect *paf, void *obj, bool fRemove) { affect_apply_stat(paf, obj, fRemove, STAT_CON); }

void affect_apply_luck(Affect *paf, void *obj, bool fRemove) { affect_apply_stat(paf, obj, fRemove, STAT_LUCK); }
...
void affect_apply_align(Affect *paf, void *obj, bool fRemove) {
  Character *ch = (Character *)obj;
  assert(ch != 0);
  ch->alignment = URANGE(-MAX_ALIGN, ch->alignment + affect_mod(paf, fRemove), MAX_ALIGN);
}

const Lookup affect_callbacks[] = {{"none", 0},
...
void destroy_affect(Affect *aff) { free_mem(aff); }
...
const char *affect_name(Affect *paf) { return lookup_name(affect_callbacks, (uintptr_t)paf->callback); }
  ...
  if (sql_step(stmt) != SQL_DONE) {
    ...
    for (int i = 0; i < count; i++) {
      ...
      if (!str_cmp(colname, "affectId")) {
        if (id != sql_column_int(stmt, i)) log_error("sql statement did not return correct affect");
      }
        ...
        parse_flags(paf->flags, sql_column_str(stmt, i), affect_flags);
      }

      else if (!str_cmp(colname, "type")) {
        paf->callback = (AffectCallback *)value_lookup(affect_callbacks, sql_column_str(stmt, i));
      }
...
int save_affect(Affect *paf) {
  field_map affect_values[] = {{"duration", &paf->duration, SQL_INT},
