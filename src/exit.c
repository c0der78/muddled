...
const Lookup direction_table[] = {{"north", DIR_NORTH},
                                  {"east", DIR_EAST},
                                  {"south", DIR_SOUTH},
                                  {"west", DIR_WEST},
                                  {"up", DIR_UP},
...
const Lookup exit_flags[] = {{"closed", EXIT_CLOSED}, {"locked", EXIT_LOCKED}, {0, 0}};
  ...
  for (int i = 0; i < ID_HASH; i++) {
    for (Room *room = room_hash[i]; room != 0; room = room->next) {
      for (direction_t dir = DIR_NORTH; dir < MAX_DIR; dir++) {
        ...
        if (toRoom == 0) log_bug("invalid room (%" PRId64 ") for exit (%" PRId64 ")", id, room->exits[dir]->id);
  ...
  while (sql_step(stmt) != SQL_DONE) {
    ...
    for (int i = 0; i < count; i++) {
      ...
      if (!str_cmp(colname, "exitId")) {
        ...
        if (room->id != sql_column_int(stmt, i)) log_error("sql returned invalid exit for room");
      }
        ...
        parse_flags(exit->flags, sql_column_str(stmt, i), exit_flags);
...
int save_exit(Exit *exit, direction_t dir) {
  if (exit->to.room == 0) {
    log_error("Exit %" PRId64 " with no room %" PRId64, exit->id, exit->to.id);
    return 0;
  }
  field_map exit_values[] = {{"toRoom", &exit->to.room->id, SQL_INT},
