...
const Lookup dam_types[] = {{"bash", DAM_BASH},
                            {"pierce", DAM_PIERCE},
                            {"slash", DAM_SLASH},
                            {"acid", DAM_ACID},
                            {"cold", DAM_COLD},
...
void dam_message(Character *ch, Character *victim, long dam, int dt, dam_t type) {
  const char *attack = (dt >= 0 && dt < max_skill) ? skill_table[dt].damage : "attack";
  ...
  if (dam <= 0) {
    ...
    actf(TO_CHAR, ch, attack, victim, "Your $t hits $N for %ld damage!", dam);
    actf(TO_VICT, ch, attack, victim, "$n's $t hits you for %ld damage!", dam);
    actf(TO_NOTVICT, ch, attack, victim, "$n's $t hits $N for %ld damage!", dam);
  }
}
...
long xp_compute(Character *gch, Character *victim, int total_levels) {
   ...
   */
  int xp_group = (exp_table[UMIN(MAX_PLAYABLE_LEVEL, gch->level) / LEVEL_GROUPS] / LEVEL_GROUPS);
  xp *= xp_group;
  /*
...
void advance_level(Character *ch, bool hide) {
   ...
   * 0]); //set_title(ch, buf);
   */
  add_hp = getCurrStat(ch, STAT_CON) / exp_table[UMIN(MAX_PLAYABLE_LEVEL, ch->level) / LEVEL_GROUPS];
  add_mana = number_range(getCurrStat(ch, STAT_INT), (2 * getCurrStat(ch, STAT_INT) + getCurrStat(ch, STAT_WIS)) / 5);

  if (*ch->classes != -1 && !class_table[*ch->classes].fMana) {
    add_mana /= 2;
  }
  add_move = number_range(getCurrStat(ch, STAT_DEX), (getCurrStat(ch, STAT_CON) + getCurrStat(ch, STAT_DEX)) / 6);
  ...
  if (!hide) {
    xwritelnf(ch, "You gain %d hit point%s, %d mana, %d move.", add_hp, add_hp == 1 ? "" : "s", add_mana, add_move);
  }
  return;
}
...
bool damage(Character *ch, Character *victim, long dam, int dt, dam_t type) {
  ...
  if (victim->hit <= 0) {
    ...
    if (victim->pc == 0) {
      ...
      victim->fighting = 0;
      announce(victim, INFO_DEATH, "Alas, $n has been killed by %s.", NAME(ch));
    }
    return true;
  }
  return true;
...
void one_hit(Character *ch, Character *victim, int dt, dam_t type) {
  ...
  int thac0 = interpolate(ch->level, class_table[*ch->classes].thac0, class_table[*ch->classes].thac32);
  ...
  int victim_ac = type == DAM_UNDEFINED ? DAM_UNDEFINED : victim->resists[type];
  long diceroll;
  ...
  dam += number_range(getCurrStat(ch, STAT_STR) / 2, getCurrStat(ch, STAT_STR));
