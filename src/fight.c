...
long xp_compute( Character * gch, Character * victim, int total_levels )
{
	...
	int xp_group = ( exp_table[UMIN( MAX_PLAYABLE_LEVEL, gch->level ) / LEVEL_GROUPS] /
		  LEVEL_GROUPS );
...
void advance_level( Character * ch, bool hide )
{
	...
	add_move =
		number_range( getCurrStat( ch, STAT_DEX ),
					  ( getCurrStat( ch, STAT_CON )
						+ getCurrStat( ch, STAT_DEX ) ) / 6 );
