...
long number_mm(void) { return rand() >> 6; }
...
long number_bits(int width) { return number_mm() & ((1 << width) - 1); }
...
long number_fuzzy(long num, int fuzziness) { return number_range(num - fuzziness, num + fuzziness); }
...
bool can_see(const Character *ch, const Character *ch2) { return true; }

bool can_see_room(const Character *ch, const Room *room) { return true; }

bool can_see_obj(const Character *ch, const Object *obj) { return true; }
...
void equip_char(Character *ch, Object *obj, int iWear) {
  ...
  if (get_eq_char(ch, iWear) != NULL) {
    log_bug("char %" PRId64 " already equipped (obj %" PRId64 ") in room %" PRId64 " (%s).", ch->id, obj->id,
            ch->inRoom ? ch->inRoom->id : 0, wear_types[iWear].name);
    return;
  }
  /*
...
void act(int type, const Character *ch, const void *arg1, const void *arg2, const char *orig) {
  act_pos(type, POS_RESTING, ch, arg1, arg2, orig);
}

void act_pos(int type, position_t minPos, const Character *ch, const void *arg1, const void *arg2, const char *orig) {
  ...
  for (; to != NULL; to = (type & TO_WORLD) ? to->next_player : to->next_in_room) {
    ...
    while (*str != 0) {
      ...
      if (!arg2 && isupper((int)*str)) {
        log_error("act: missing arg2 for code %d.", *str);
      }
...
void actf(int type, const Character *ch, const void *arg1, const void *arg2, const char *fmt, ...) {
  va_list args;
  char buf[BUF_SIZ];
  va_start(args, fmt);
...
void actf_pos(int type, position_t minPos, const Character *ch, const void *arg1, const void *arg2, const char *fmt,
              ...) {
  va_list args;
  char buf[BUF_SIZ];
  va_start(args, fmt);
...
const struct timezone_type timezones[] = {{"GMT-12", "Eniwetok", -12, 0},
                                          {"GMT-11", "Samoa", -11, 0},
                                          {"GMT-10", "Hawaii", -10, 0},
                                          {"GMT-9", "Alaska", -9, 0},
...
int timezone_lookup(const char *arg) {
  ...
  for (int i = 0; timezones[i].name != 0; i++) {
    ...
    else if (!str_cmp(arg, timezones[i].name)) {
      return i;
    }
  }
  return -1;
...
const char *str_time(time_t timet, int tz, const char *format) {
  ...
  strftime(result, 100,
           ...
           format != NULL ? format : "%a %b %d %I:%M:%S %p %Y", localtime(&timet));
  return result;
}
...
int gold(money_t value) { return (int)value; }

int silver(money_t value) { return (int)((float)(value - (int)value) * 100.0); }
