...
void obj_from_obj( Object * obj )
{
	...
	/*
	   for ( ; obj_from != NULL; obj_from = obj_from->inObj )
	   {
...
void equip_char( Character * ch, Object * obj, int iWear )
{
	...
	if ( get_eq_char( ch, iWear ) != NULL )
	{
		...
		log_bug( "char %"PRId64" already equipped (obj %"PRId64") in room %"PRId64" (%s).",
				 ch->id, obj->id, ch->inRoom ? ch->inRoom->id : 0,
...
const char *chview( const Character * ch, const Character * looker )
{
	...
	/*
	   if (viewer can't see ch) {
	   strcpy(buf[index], "someone");
	   }
	   else
	 */
...
const char *objview( const Object * obj, const Character * looker )
{
	...
	/*
	   if (viewer can't see obj) {
	   strcpy(buf[index], "something");
	   }
	   else
	 */
...
void actf( int type, const Character * ch,
...
void actf_pos( int type, position_t minPos, const Character * ch,
			   const void *arg1, const void *arg2, const char *fmt, ... )
{
	va_list args;
...
const char *str_time( time_t timet, int tz, const char *format )
{
	...
	if ( tz > -1 && tz < ( sizeof( timezones ) / sizeof( timezones[0] ) ) - 1 )
	{
		...
		timet += ( 60 * 60 * timezones[tz].gmt_offset );	/* Add the offset hours */
